...
def FK_solver(x, u, pars, sensor_noise=False, SNR_DB=0, i=0):
    ...
    if not f_origin:
        f_drift = np.kron(vel, np.array([[1], [0]])) + np.kron(-m * g * l / J * np.sin(pos) - (gamma / J) * vel,
                                                               np.array([[0], [1]]))
    else:
        f_drift = np.kron(vel, np.array([[1], [0]])) + np.kron(m * g * l / J * np.sin(pos) - (gamma / J) * vel,
                                                               np.array([[0], [1]]))
    ...
    dxdt = (f_drift + (A_inter @ x.reshape((2 * N, -1))) + controlled_input) * (
                1 + noise * 10 ** (-SNR_DB / 20)) + uncertainty_factor
