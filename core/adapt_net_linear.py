...
class AdaptNet_linear(nn.Module):
    def __init__(self, adapt_net_params, device=None):
    ...
    def construct_adapt_net(self):
        ...
        if override_C:
            self.n_tot = first_obs_const + lift_dim + n
        else:
            self.n_tot = first_obs_const + lift_dim

        self.del_A = nn.Linear(self.n_tot, self.n_tot - first_obs_const, bias=False).to(self.device)
        self.del_B = nn.Linear(m, self.n_tot - first_obs_const, bias=False).to(self.device)

        if self.adapt_net_params['warm_start'] is True:
            del_A_prev = torch.from_numpy(
                self.adapt_net_params['del_A_prev'][first_obs_const:, :]
            ).float().to(self.device)

            del_B_prev = torch.from_numpy(
                self.adapt_net_params['del_B_prev'][first_obs_const:, :]
            ).float().to(self.device)
    ...
    def forward(self, data):
        ...
        del_z_pred = (
            torch.matmul(Z, del_A_matrix.transpose(0, 1))
            + torch.matmul(U, del_B_matrix.transpose(0, 1))
        )
        return del_z_pred
    ...
    def construct_del_matrix_(self):
        ...
        const_obs_dyn_drift = torch.zeros((first_obs_const, self.n_tot), device=self.device)
        self.del_A_matrix = torch.cat((const_obs_dyn_drift, self.del_A.weight), dim=0)

        const_obs_dyn_act = torch.zeros((first_obs_const, m), device=self.device)
        self.del_B_matrix = torch.cat((const_obs_dyn_act, self.del_B.weight), dim=0)
    ...
    def loss(self, outputs, labels):
        ...
        loss = (
            ...
            + alpha * torch.norm(self.del_A_matrix, p=1)
            + alpha * torch.norm(self.del_B_matrix, p=1)
            + beta * torch.norm(self.del_A_matrix, p=2)
            + beta * torch.norm(self.del_B_matrix, p=2)
        )
        return loss
    ...
    def train_model(self, dataset_train, print_epoch=True):
        trainloader = torch.utils.data.DataLoader(
            dataset_train,
            batch_size=self.adapt_net_params['batch_size'],
            shuffle=False,
        ...
        for epoch in range(self.adapt_net_params['epochs']):
            ...
            if print_epoch:
                print(f"[Adapt] Epoch {epoch+1:3d}: loss = {epoch_loss:.10f}")

    def set_optimizer_(self):
        opt_name = self.adapt_net_params['optimizer']
    ...
    def get_del_matrices(self):
        del_A_matrix = self.del_A_matrix.detach().cpu().numpy()
        del_B_matrix = self.del_B_matrix.detach().cpu().numpy()
