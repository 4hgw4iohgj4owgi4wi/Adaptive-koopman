    ...
    try:
        for k in range(sim_steps_cap):
            ...
            if use_comm_quality_consensus:
                if use_comm_restructure:
                    # Drive the protection state from the exact packets delivered
                    # to the distributed controllers.  Sampling a second hidden
                    # channel here would make quality/fallback react to events that
                    # did not occur on the plant-facing communication trace.
                    pair_rows = list(comm_perception_diag.get("relative_pair_diag", []))
                    observed_delay = [int(row.get("delay_steps", 0)) for row in pair_rows]
                    observed_ok = [not bool(row.get("dropped", False)) for row in pair_rows]
                    default_delay = int(round(float(np.mean(observed_delay)))) if observed_delay else 0
                    default_ok = bool(float(np.mean(observed_ok)) >= 0.5) if observed_ok else True
                    pair_lookup = {
                        (int(row.get("sender", -1)), int(row.get("receiver", -1))): row
                        for row in pair_rows
                    }
                    for sender in range(num_vehicles):
                        for receiver in range(num_vehicles):
                            if sender == receiver:
                                comm_consensus.packet_ok[sender, receiver] = True
                                comm_consensus.delay_steps[sender, receiver] = 0
                                comm_consensus.quality[sender, receiver] = 1.0
                                continue
                            observed = pair_lookup.get((sender, receiver), None)
                            delay_value = (
                                int(observed.get("delay_steps", default_delay))
                                if observed is not None
                                else default_delay
                            )
                            packet_ok_value = (
                                not bool(observed.get("dropped", not default_ok))
                                if observed is not None
                                else default_ok
                            )
                            comm_consensus.delay_steps[sender, receiver] = max(0, delay_value)
                            comm_consensus.packet_ok[sender, receiver] = bool(packet_ok_value)
                            q_inst = np.exp(
                                -float(max(0, delay_value)) / comm_consensus.quality_tau_steps
                            )
                            if not packet_ok_value:
                                q_inst *= 0.10
                            beta_q = comm_consensus.quality_smooth_beta
                            q_prev = float(comm_consensus.quality[sender, receiver])
                            comm_consensus.quality[sender, receiver] = float(
                                np.clip(beta_q * q_prev + (1.0 - beta_q) * q_inst, 0.0, 1.0)
                            )
                    comm_consensus.last_diag = {
                        "quality_global": comm_consensus._global_quality(),
                        "mean_delay_steps": comm_consensus._global_delay(),
                        "loss_ratio": comm_consensus._loss_ratio(),
                        "source": "delivered_restructure_trace",
                    }
                    comm_step_diag = dict(comm_consensus.last_diag)
                else:
