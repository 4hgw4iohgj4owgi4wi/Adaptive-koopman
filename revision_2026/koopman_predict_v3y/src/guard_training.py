...
def train_phase(model,fit,inner,anchor_fit,anchor_inner,method,seed,identity,out,max_steps=15000,stop_at=None,resume=None,monitor_every=500,min_steps=2000,patience=2000,deadline=None,anchor_scale_m20=None,diagnostic_callback=None):
    ...
    if resume:
        ...
        lam=payload['lambda'].to(fit.device);step=payload['step'];best=payload['best'];best_q=payload['best_q'];best_step=payload['best_step'];no_improve=payload['no_improve'];last_monitor=payload['last_monitor'];curve=payload['curve'];sample_hash=payload['sample_hash'];elapsed=payload['elapsed_s'];dual_updates=payload.get('dual_updates',0)
    start=time.perf_counter();last100=start;status='COMPLETE'
    kc_method = method in ('aligned', 'fixed_guard', 'adaptive_guard', 'residual_curriculum')
    curriculum_switch = 1000

    def pack():
        return dict(identity=identity,model=model.state_dict(),optimizer=optimizer.state_dict(),rng=rng_state(),sampler=copy.deepcopy(rng.bit_generator.state),lambda_=None,**{'lambda':lam},step=step,best=best,best_q=best_q,best_step=best_step,no_improve=no_improve,last_monitor=last_monitor,curve=curve,sample_hash=sample_hash,elapsed_s=elapsed+time.perf_counter()-start,dual_updates=dual_updates,updates_completed=dual_updates,updates_applied_to_weights=max(0,(step-1)//monitor_every) if method=='adaptive_guard' and step else 0,curriculum_phase='one_step_mse' if method=='residual_curriculum' and step<=1000 else 'multi_step')
    if diagnostic_callback is not None and step==0:
        diagnostic_callback(model,step)
    while step<max_steps:
        ...
        if kc_method:
            from refine_loss import loss_kc, residual_loss
            if method == 'residual_curriculum' and step < curriculum_switch:
                objective = residual_loss(model, fit, ix)
            else:
                eff = 'fixed_guard' if method == 'residual_curriculum' else method
                objective = loss_kc(model, fit, ix, eff, anchor_fit, lam, anchor_scale_m20 if anchor_scale_m20 is not None else float(anchor_fit[:, 3, 0].mean()))
        else:
        ...
        if diagnostic_callback is not None and step in (100,250,500,750,2500,6000):
            saved_rng=rng_state()
            try:
                diagnostic_callback(model,step)
            finally:
        ...
        if step%monitor_every==0:
            started=time.perf_counter();fit_g=None;stats=None;new_best=False;new_q=False;lambda_before=None
            if method!='warm':
                lambda_before=lam.detach().cpu().tolist()
                if kc_method:
                    from refine_loss import full_fit_constraints
                    g_fit,m20_fit=full_fit_constraints(model,fit,anchor_fit)
                    fit_g=g_fit.detach().cpu().tolist()
                    if method=='adaptive_guard':
                        lam=(lam+.5*g_fit.double()).clamp(0,20)
                        dual_updates+=1
                elif method=='T2':
                ...
                if new_q:
                    best_q=q
                last_monitor=step
            item=dict(step=step,objective=float(objective.detach()),grad_norm=float(norm),best=best,best_step=best_step,best_q=best_q,lambda_values=lam.detach().cpu().tolist(),lambda_used_before=lambda_before,lambda_after=lam.detach().cpu().tolist() if lambda_before is not None else None,dual_updates=dual_updates,fit_g=None if fit_g is None else (fit_g if isinstance(fit_g,list) else fit_g.detach().cpu().tolist()),inner_g=None if stats is None else constraints(stats,anchor_inner).detach().cpu().tolist(),monitor_s=time.perf_counter()-started,elapsed_s=elapsed+time.perf_counter()-start)
            curve.append(item)
            payload=pack()
    ...
    return dict(status=status,step=step,best_step=best_step,elapsed_s=payload['elapsed_s'],curve=curve,best=best,sample_hash=sample_hash,dual_updates=dual_updates)
