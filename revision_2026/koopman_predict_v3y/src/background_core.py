...
class QualityPolicy:
    """Explicit protection margins; never mutate the legacy Q1 thresholds."""
...
LEGACY_3_5 = QualityPolicy("legacy_3_5", .03, .03, .05, .03, (0., .25, .5, .75, 1.))
INTERNAL_2_4 = QualityPolicy("internal_2_4", .02, .02, .04, .02,
    tuple(round(i / 10, 1) for i in range(11)))


def policy_constraints(stats, anchor, policy=LEGACY_3_5):
    """The frozen 55 guards with thresholds supplied explicitly."""
    s = torch.as_tensor(stats)
    a = torch.as_tensor(anchor, dtype=s.dtype, device=s.device)
    m, b = s.mean(0), a.mean(0)
    short = (m[:3, 0] - b[:3, 0]) / b[:3, 0].clamp_min(1e-12) - policy.short_limit
    scenario = ((s[:, :, 0] - a[:, :, 0]) / a[:, :, 0].clamp_min(.02)
                - policy.scenario_limit).reshape(-1)
    physical = torch.stack([(m[h, c] - b[h, c]) / b[h, c].clamp_min(1e-12)
                            - policy.physical_limit for c in (3, 4) for h in (0, 3)])
...
def complete_quality(candidate, anchor, policy=LEGACY_3_5):
    ...
    g = policy_constraints(torch.from_numpy(s), torch.from_numpy(a), policy).numpy()
    ...
    for hidx in (1,2):
        for c, name in ((3,"force"),(4,"internal")):
            ...
            add(f"{name}_h{H[hidx]}", val, base, max(base,1e-12), policy.physical_limit)
    for subset in ("all", "hard"):
        ...
        for key,v,b in zip(("mean","p95","p99","max"), cv,av):
            add(f"hairpin20_{subset}_{key}", v,b,max(b,.02),policy.tail_limit)
    ...
    return dict(policy=policy.name, passed=all(r["passed"] for r in out), finite=True, rows=out,
                m20=float(s[:,3,0].mean()), i20=100*(base-float(s[:,3,0].mean()))/max(base,1e-12))


def select_gamma(records, policy=LEGACY_3_5):
    expected={float(x) for x in policy.gamma_grid}
    if len(records)!=len(expected) or {float(r["gamma"]) for r in records}!=expected:
        raise ValueError("incomplete gamma grid")
    if any(not np.isfinite(float(r[k])) for r in records for k in ("gamma", "m20", "i20")):
        raise ValueError("nonfinite gamma record")
    ...
    for r in options[1:]:
        if r["m20"] < best["m20"]-1e-10 or (abs(r["m20"]-best["m20"])<=1e-10 and r["gamma"]<best["gamma"]):
            best = r
    fallback = "PROTECTED_BUT_LOW_GAIN" if policy is LEGACY_3_5 else "PROTECTED_BUT_NO_GAIN"
    return dict(best, status="CANDIDATE" if good else fallback)
