...
def fit_normalization(entries,guard,fold):
    keys=('relative_state47','actual_steering4','control7','force_payload_body8','internal_force8')
    values={k:[] for k in keys}
    for e in entries:
        cache=guard.load(e,fold,'normalization')
        assert len(cache['relative_state47'])==len(cache['control7'])+1
        for k in keys:
...
def fit_s0(entries,norm,guard,fold):
    ...
    gram=np.zeros((55,55));cross=np.zeros((55,47));n=0;layout=None
    for e in entries:
...
def new_model(coeff,seed):
    from lift import TriangularResidualKoopman
    torch.manual_seed(seed)
    return TriangularResidualKoopman(coeff[:47].T,coeff[47:54].T,coeff[54],16,8,16)
    ...
    def __init__(self,entries,norm,guard,fold,purpose,params_resolver,decoder,device='cpu',horizon=20):
        ...
        for e in entries:
            ...
            for start in c['window_start'].astype(int):
                if start+horizon>=len(c['relative_state47']) or start+horizon>len(c['control7']):
                    self.missing+=1
                    if horizon==20:
                ...
                us.append((c['control7'][start:start+horizon]-norm['control7_mean'])/norm['control7_scale'])
