...
def _horizon_error(prediction_h, target_h, group_weights, batch):
    """Group error for one horizon; scenario-balanced when the batch carries
    window_weights, otherwise the plain v3r group_rmse (bitwise identical path)."""
    window_weights = batch.get("window_weights")
    if window_weights is None:
        return group_rmse(prediction_h, target_h, group_weights)
    return group_rmse_window_weighted(prediction_h, target_h, group_weights, window_weights)


def build_scenario_weights(dataset: FrozenSequenceDataset, protocol: dict, mode: str) -> dict:
    """SB16 per-window scenario weights (koopman_ab.md SB1):

    w_s = N_train / (12 * n_s^train)          (equal_scenario)
    w'_s = factor * w_s for s in up.scenarios, then renormalised to mean 1
                                             (up_weight; factor/scenarios from protocol)

    Weights derive from train window counts only; no future/validation info.
    Returns {"mode": mode, "weights": {scenario: float}, "counts": {...},
             "n_total": int, "weighted_mean": float}.
    """
    sb = protocol["loss"]["scenario_balance"]
    counts = dataset.scenario_window_counts()
    n_total = sum(counts.values())
    denominator_scenarios = float(sb["denominator_scenarios"])
    weights = {scenario: n_total / (denominator_scenarios * max(count, 1)) for scenario, count in counts.items()}
    if mode == "up_weight":
        up_cfg = sb["modes"]["up_weight"]
        up_scenarios = [str(s) for s in up_cfg["scenarios"]]
        factor = float(up_cfg["factor"])
        for scenario in up_scenarios:
            if scenario in weights:
                weights[scenario] = factor * weights[scenario]
    mean_w = sum((counts[s] / n_total) * weights[s] for s in weights)
    weights = {s: w / mean_w for s, w in weights.items()}
    weighted_mean = sum((counts[s] / n_total) * weights[s] for s in weights)
    return {
        "mode": mode,
        "weights": weights,
        "counts": counts,
...
def step_loss(
    ...
    if one_step_spec:
        per_horizon = {1: _horizon_error(predictions[:, 0], targets[:, 0], group_weights, batch)}
    else:
        per_horizon = {
            horizon: _horizon_error(predictions[:, horizon - 1], targets[:, horizon - 1], group_weights, batch)
...
def train_phase(
    ...
    *,
    ...
    resume: bool = False,
    scenario_balance: dict | None = None,
) -> dict:
    """Unified training loop over a position-indexed batch stream.

    scenario_balance (SB16): {"mode": ..., "weights": {scenario: float}} as
    returned by build_scenario_weights; when set, every phase batch carries
    per-window weights so step_loss uses the balanced group error.  The shared
    warm phase runs without balancing (plain one_step)."""
    ...
    scenario_weight_map = None
    if scenario_balance is not None:
        scenario_weight_map = {str(s): float(w) for s, w in scenario_balance["weights"].items()}
    ...
    while step < int(steps):
        ...
        if scenario_weight_map is not None:
            batch["window_weights"] = torch.as_tensor(
                [scenario_weight_map[str(meta["scenario"])] for meta in batch["metadata"]],
