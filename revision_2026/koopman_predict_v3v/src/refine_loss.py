...
def residual_loss(model, data, ix):
    ...
    pred1 = model.rollout(data.x[ix].float(), data.u[ix].float(), (1,))["xhat"][:, 0]
    err = pred1 - data.y[ix][:, 0].float()
    g0 = smooth_rmse(err[:, 0:3].reshape(-1))
    g1 = smooth_rmse(err[:, 3:19].reshape(-1))
    g2 = smooth_rmse(err[:, 19:31].reshape(-1))
    g3 = smooth_rmse(err[:, 31:47].reshape(-1))
    val = 0.30 * g0 + 0.20 * g1 + 0.20 * g2 + 0.30 * g3
    reg = 1e-4 * (model.E**2).sum() + 1e-5 * sum((p**2).sum() for p in model.parameters())
    return val + reg
