...
INPUT_KEYS_FOCUS = INPUT_KEYS_V4 + (
...
LABEL_KEYS_FOCUS = LABEL_KEYS_V4 + (
    "payload_support_load4_k1",
    "vehicle_total_normal_load4_k1",
    "payload_accel_body2_k1",
    "icr_geom_k1",
    "icr_request_k1",
    "icr_actual_k1",
)
...
INPUT_KEYS_ICR = INPUT_KEYS_FOCUS + (
    "actuator_sidecar20_k",
    "actuator_gate8_k",
)


LABEL_KEYS_ICR = LABEL_KEYS_FOCUS


INPUT_KEYS_PREDICT = {
    "S0": ("state47_k", "virtual_control3_k", "steering_request4_k"),
    "S1": ("hybrid_state51_k", "virtual_control3_k", "steering_request4_k"),
    "S2": (
        "hybrid_state51_k",
        "virtual_control3_k",
        "steering_request4_k",
...
LABEL_KEYS_PREDICT = {
    "S0": ("state47_k1",),
    "S1": ("hybrid_state51_k1",),
    "S2": ("hybrid_state51_k1",),
}
    ...
    *,
    ...
    sample = {
        **base,
        ...
        "analytic_actual_steering4_k1": np.asarray(
            analytic["delta_act_k1_rad"], dtype=float
        ).copy(),
...
def _predict_variant(variant: str) -> str:
    selected = str(variant).upper()
    if selected not in INPUT_KEYS_PREDICT:
        raise ValueError(f"unknown predict sample variant: {variant}")
    return selected


def build_sample_variant(
    arrays: Mapping[str, np.ndarray],
    index: int,
    params: object,
    *,
    ...
    """Build one frozen S0/S1/S2 sample from causal raw endpoints.

    The control at raw row ``index + 1`` is the request held over the
    ``index -> index + 1`` model interval.  S2's next actuator state is the A3
    analytic result from the current measured state and that known request;
    the recorded next actuator endpoint remains label-only.
    """

    selected = _predict_variant(variant)
    required = {"virtual_front_deg", "virtual_rear_deg"}
    if "base_acceleration_mps2" not in arrays and "acceleration_request_mps2" not in arrays:
        required.add("base_acceleration_mps2")
    missing = sorted(required - set(arrays))
    if missing:
        raise KeyError(f"missing predict raw arrays: {missing}")
    base = build_sample_v4(
        arrays,
        index,
        params,
        expected_step_s=expected_step_s,
    ...
    acceleration_key = (
        "acceleration_request_mps2"
        if "acceleration_request_mps2" in arrays
        else "base_acceleration_mps2"
    )
    acceleration = float(np.asarray(arrays[acceleration_key], dtype=float)[index + 1])
    virtual = np.asarray(
        [
            acceleration,
            np.deg2rad(float(np.asarray(arrays["virtual_front_deg"])[index + 1])),
            np.deg2rad(float(np.asarray(arrays["virtual_rear_deg"])[index + 1])),
        ],
        dtype=float,
    )
    relative_k = np.asarray(base["relative_state47_k"], dtype=float)
    relative_k1 = np.asarray(base["relative_state47_k1"], dtype=float)
    actual_k = np.asarray(base["actual_steering4_k"], dtype=float)
    actual_k1 = np.asarray(base["actual_steering4_k1"], dtype=float)
    rate_mask = np.asarray(base["actuator_rate_limited4_k"], dtype=bool)
    angle_mask = np.asarray(base["actuator_angle_limited4_k"], dtype=bool)
    sample: dict[str, np.ndarray | float | int | str] = {
        "variant": selected,
        "state47_k": relative_k.copy(),
        "state47_k1": relative_k1.copy(),
        "hybrid_state51_k": np.r_[relative_k, actual_k],
        "hybrid_state51_k1": np.r_[relative_k1, actual_k1],
        "virtual_control3_k": virtual,
        "steering_request4_k": np.asarray(base["steering_request4_k"], dtype=float).copy(),
        "analytic_actual_steering4_k1": np.asarray(
            base["analytic_actual_steering4_k1"], dtype=float
        ).copy(),
        "analytic_rate_mask4_k": rate_mask.copy(),
        "analytic_angle_mask4_k": angle_mask.copy(),
        "sample_time_s": float(base["sample_time_s"]),
        "target_time_s": float(base["target_time_s"]),
        "source_index": int(base["source_index"]),
    }
    for key in INPUT_KEYS_PREDICT[selected] + LABEL_KEYS_PREDICT[selected]:
        value = np.asarray(sample[key])
        if not np.all(np.isfinite(value)):
            raise ValueError(f"non-finite {selected} sample field: {key}")
    return sample


def causal_input_digest_variant(sample: Mapping[str, object], variant: str) -> str:
    selected = _predict_variant(variant)
    digest = hashlib.sha256()
    for key in INPUT_KEYS_PREDICT[selected]:
        value = np.ascontiguousarray(np.asarray(sample[key]))
        digest.update(key.encode("utf-8"))
        digest.update(str(value.dtype).encode("ascii"))
        digest.update(np.asarray(value.shape, dtype=np.int64).tobytes())
...
def build_sample_focus(
    ...
    *,
    ...
    """Build focus inputs by recomputing current support and ICR context causally."""
    ...
    required = {
        ...
        "load_transfer_enabled",
    }
    missing = sorted(required - set(arrays))
    if missing:
        raise KeyError(f"missing focus raw arrays: {missing}")
    ...
    from four_vehicle_common import connector_diagnostics, rotation, split_state
    from load_transfer import config_from_model, solve_payload_support_loads
    from steering_allocator import kinematic_targets

    state_k = np.asarray(sample["physical_state30_k"], dtype=float)
    _, payload = split_state(state_k)
    connector = connector_diagnostics(state_k, params, law)
    force_world = np.sum(connector["force_payload_world_n"], axis=0)
    payload_accel = rotation(float(payload[2])).T @ (
        force_world / float(params.payload.mass_kg)
    )
    load_enabled = bool(np.asarray(arrays["load_transfer_enabled"]).reshape(-1)[0])
    ...
    target = kinematic_targets(
        ...
        virtual_front_rad=np.deg2rad(float(arrays["virtual_front_deg"][index + 1])),
        virtual_rear_rad=np.deg2rad(float(arrays["virtual_rear_deg"][index + 1])),
        params=params,
    )
    geometry = icr_geometry_residual(
    ...
    request = icr_steering_residual(
        target["speed_mps"],
        float(target["yaw_rate_radps"]),
        wheelbase,
        np.asarray(sample["steering_request4_k"], dtype=float),
    )
    actual = icr_steering_residual(
        target["speed_mps"],
        float(target["yaw_rate_radps"]),
        wheelbase,
        np.asarray(sample["actual_steering4_k"], dtype=float),
    )
    support_k = np.asarray(support["payload_support_load_n"], dtype=float)
    sample.update(
        {
            "payload_support_load4_k": support_k.copy(),
            "vehicle_total_normal_load4_k": (
                float(params.vehicle.mass_kg * params.vehicle.gravity_mps2) + support_k
            ),
            "payload_accel_body2_k": payload_accel.copy(),
            "icr_geom_k": np.asarray([float(geometry["max_abs_mps"])]),
            "icr_request_k": np.asarray([float(request["max_abs_mps"])]),
            "icr_actual_k": np.asarray([float(actual["max_abs_mps"])]),
            "payload_support_load4_k1": np.asarray(
                arrays["payload_support_load4_n"][index + 1], dtype=float
            ).copy(),
            "vehicle_total_normal_load4_k1": np.asarray(
                arrays["vehicle_total_normal_load4_n"][index + 1], dtype=float
            ).copy(),
            "payload_accel_body2_k1": np.asarray(
                arrays["payload_accel_body2_mps2"][index + 1], dtype=float
            ).copy(),
            "icr_geom_k1": np.asarray(
                [float(arrays["icr_geometry_residual_mps"][index + 1])]
            ),
            "icr_request_k1": np.asarray(
                [float(arrays["icr_request_residual_mps"][index + 1])]
            ),
            "icr_actual_k1": np.asarray(
                [float(arrays["icr_actual_residual_mps"][index + 1])]
            ),
        }
    )
    for key in INPUT_KEYS_FOCUS + LABEL_KEYS_FOCUS:
        value = np.asarray(sample[key])
        if not np.all(np.isfinite(value)):
            raise ValueError(f"non-finite focus sample field: {key}")
    return sample


def causal_input_digest_focus(sample: Mapping[str, object]) -> str:
    digest = hashlib.sha256()
    for key in INPUT_KEYS_FOCUS:
        value = np.ascontiguousarray(np.asarray(sample[key]))
        digest.update(key.encode("utf-8"))
        digest.update(str(value.dtype).encode("ascii"))
        digest.update(np.asarray(value.shape, dtype=np.int64).tobytes())
...
def build_sample_icr(
    ...
    *,
    ...
    """Build the causal actuator sidecar required by future Koopman stages."""
    ...
    actual = np.asarray(sample["actual_steering4_k"], dtype=float)
    rate = np.asarray(sample["actual_steering_rate4_prev"], dtype=float)
    error = np.asarray(sample["actuator_error4_k"], dtype=float)
    rate_mask = np.asarray(sample["actuator_rate_limited4_k"], dtype=bool)
    angle_mask = np.asarray(sample["actuator_angle_limited4_k"], dtype=bool)
    delta_request = np.abs(
        np.asarray(sample["delta_u_req8_k"], dtype=float).reshape(4, 2)[:, 1]
    )
    sidecar = np.r_[
        actual,
        rate,
        error,
        rate_mask.astype(float),
        angle_mask.astype(float),
    ]
    gate = np.r_[
        delta_request,
        np.linalg.norm(error),
        float(np.asarray(sample["icr_actual_k"]).reshape(-1)[0]),
        float(np.max(rate_mask)),
        float(np.max(angle_mask)),
    ]
    sample.update(
        {
            "actuator_sidecar20_k": sidecar,
            "actuator_gate8_k": gate,
        }
    )
    for key in INPUT_KEYS_ICR + LABEL_KEYS_ICR:
        value = np.asarray(sample[key])
        if not np.all(np.isfinite(value)):
            raise ValueError(f"non-finite ICR sample field: {key}")
    return sample


def causal_input_digest_icr(sample: Mapping[str, object]) -> str:
    digest = hashlib.sha256()
    for key in INPUT_KEYS_ICR:
        value = np.ascontiguousarray(np.asarray(sample[key]))
        digest.update(key.encode("utf-8"))
        digest.update(str(value.dtype).encode("ascii"))
        digest.update(np.asarray(value.shape, dtype=np.int64).tobytes())
