...
def freeze(
    *,
    ...
    taskbook_path: Path,
    output: Path,
    parent_n3: dict | None = None,
) -> dict:
    manifest = source_manifest(source_root)
    repeated_manifest = source_manifest(source_root)
    repeated_sha = json_sha256(repeated_manifest)
    manifest_sha = json_sha256(manifest)
    if manifest != repeated_manifest or manifest_sha != repeated_sha:
        raise RuntimeError("source manifest is not stable across consecutive scans")
    environment = environment_manifest(project_root)
    result = {
        "source_root": str(source_root),
        "source_manifest": manifest,
        "source_manifest_sha256": manifest_sha,
        "source_manifest_repeat_sha256": repeated_sha,
        "consecutive_manifest_match": True,
        ...
        "excluded_from_source_manifest": [
            "__pycache__",
            ".pytest_cache",
            "runs/results/models/checkpoints",
            "*.pyc/*.tmp/*.lock/*.partial",
            "non_py_json_md_files",
        ],
    }
