...
def group_rmse_window_weighted(
    prediction: torch.Tensor,
    target: torch.Tensor,
    group_weights: dict[str, tuple[float, slice]],
    window_weights: torch.Tensor,
) -> torch.Tensor:
    """Scenario-balanced group RMSE (koopman_ab.md SB16):

    l_h = sum_g w_g * sqrt( mean_{batch,cols} (ww_i * err_ig^2) )

    with per-window weights ww_i (mean over the training window set == 1).
    Callers keep the plain group_rmse path when balancing is disabled, so
    disabled behaviour stays bitwise identical to the v3r loss (regression gate).
    """
    error = prediction - target
    ww = window_weights.to(dtype=error.dtype).reshape(-1, 1)
    total = None
    for _, (weight, selected) in group_weights.items():
        term = float(weight) * torch.sqrt(torch.mean(ww * error[:, selected] ** 2))
        total = term if total is None else total + term
    return total
...
def cvar_tail(
    ...
    Returns (tail_mean, member_positions): member_positions holds the indices
    into the flattened per-sample vector that formed the tail, so the training
    loop can log tail membership for the BCV audit."""
    values = per_sample_errors.reshape(-1)
    if values.numel() == 0:
        raise ValueError("empty per-sample error vector")
    keep = max(int(min_samples), int(math.ceil((1.0 - float(quantile)) * values.numel())))
    keep = min(keep, values.numel())
    top_values, top_indices = torch.topk(values, k=keep)
    return torch.mean(top_values), top_indices
