...
class RunLock:
    ...
    def __enter__(self):
        ...
        try:
            ...
            self.file.close()
            raise StopRun("another background queue holds the project lock", 22)
        return self
...
class Queue:
    ...
    def restore_budget(self):
        ...
        self.previous_s = float(read_json(p).get("elapsed_s",0)) if p.exists() else 0.
    ...
    def resource_gate(self):
        if time.monotonic()>=self.deadline:
            raise StopRun("12小时累计预算用完，保存进度后停止",30)
        if shutil.disk_usage(self.rev).free < 60*1024**3:
            raise StopRun("D盘剩余空间不足60 GiB",30)
    ...
    def context(self,fold):
        ...
        frozen_context=getattr(self,"frozen_context_run",None)
        if frozen_context is not None:
            frozen_context=Path(frozen_context).resolve()
            with np.load(frozen_context/f"folds/fold{fold}/normalization.npz",allow_pickle=False) as z:
                norm={k:z[k] for k in z.files}
            with np.load(frozen_context/f"folds/fold{fold}/s0.npz",allow_pickle=False) as z:
                s0={"coefficients":z["coefficients"]}
            write_json(self.run/f"folds/fold{fold}/frozen_context_import.json",dict(
                source=str(frozen_context),normalization_sha=digest_arrays(norm),
                s0_sha=digest_arrays(dict(coeff=s0["coefficients"])),decision="read-only frozen evaluation context"))
        else:
        ...
        if fold==0 and frozen_context is None:
            ...
            for unit in [u for u in units if u["fold"]==fold]:
                ...
                for calibrated in (False,True):
                    ...
                    if fold==0 and unit["repeat"]==0 and unit["method"]==selected["method"] and calibrated==selected["calibrated"]:
                        meta=min(outer.meta,key=lambda m:(m["family"],m["trajectory"],m["start"]))
                        self.timing_spec=dict(checkpoint=unit["checkpoint"],checkpoint_sha=unit["checkpoint_sha"],
                            fold=fold,repeat=unit["repeat"],seed=unit["seed"],gamma=gamma,
                            sample_id=[meta["family"],meta["trajectory"],int(meta["start"])],
                            normalization_sha=unit["norm_sha"])
        def merge(parts):
    ...
    def latency(self):
        ...
        from benchmark_case import build_benchmark_case
        write_json(self.run/"timing_spec.json",self.timing_spec)
        model,data=build_benchmark_case(self,self.timing_spec)
