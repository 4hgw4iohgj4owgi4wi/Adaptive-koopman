...
def assemble_derivative(state:np.ndarray,controls:np.ndarray,params:ModelParams,law:str,include_connector:bool=True)->tuple[np.ndarray,dict]:
    vehicles,payload=split_state(state);controls=np.asarray(controls,float).reshape(4,2);connector=connector_diagnostics(state,params,law);support=params.payload.mass_kg*params.payload.gravity_mps2/4.;vehicle_derivative=np.empty_like(vehicles);tires=[];connector_dx=np.zeros(30)
    for i in range(4):
        tire=tire_force(vehicles[i],controls[i,0],controls[i,1],support,params.vehicle);tires.append(tire);cf=connector["force_vehicle_body_n"][i] if include_connector else np.zeros(2);cm=connector["vehicle_moment_nm"][i] if include_connector else 0.;_,_,yaw,vx,vy,r=vehicles[i];vw=rotation(yaw)@np.asarray([vx,vy]);vehicle_derivative[i]=[vw[0],vw[1],r,(tire["force_body_n"][0]+cf[0])/params.vehicle.mass_kg+r*vy,(tire["force_body_n"][1]+cf[1])/params.vehicle.mass_kg-r*vx,(tire["yaw_moment_nm"]+cm)/params.vehicle.yaw_inertia_kgm2];base=6*i;connector_dx[base+3]=connector["force_vehicle_body_n"][i,0]/params.vehicle.mass_kg;connector_dx[base+4]=connector["force_vehicle_body_n"][i,1]/params.vehicle.mass_kg;connector_dx[base+5]=connector["vehicle_moment_nm"][i]/params.vehicle.yaw_inertia_kgm2
    pr=rotation(payload[2]);pf=np.sum(connector["force_payload_world_n"],axis=0) if include_connector else np.zeros(2);pa=pr.T@(pf/params.payload.mass_kg);vx,vy,r=payload[3:6];vw=pr@payload[3:5];pm=connector["payload_moment_total_nm"] if include_connector else 0.;pd=np.asarray([vw[0],vw[1],r,pa[0]+r*vy,pa[1]-r*vx,pm/params.payload.yaw_inertia_kgm2]);full=np.r_[vehicle_derivative.ravel(),pd];connector_dx[27:29]=pr.T@(np.sum(connector["force_payload_world_n"],axis=0)/params.payload.mass_kg);connector_dx[29]=connector["payload_moment_total_nm"]/params.payload.yaw_inertia_kgm2
    return full,{"connectors":connector,"tire":tires,"payload_accel_body_mps2":pa,"connector_derivative":connector_dx,"include_connector":include_connector}


def system_derivative(state: np.ndarray, controls: np.ndarray, params: ModelParams, law: str) -> tuple[np.ndarray, dict]:
    return assemble_derivative(state,controls,params,law,True)


def rk4_step(state: np.ndarray, controls: np.ndarray, dt: float, params: ModelParams, law: str) -> np.ndarray:
    def rhs(value: np.ndarray) -> np.ndarray:
        return system_derivative(value, controls, params, law)[0]
