...
if ($device.device -notmatch 'RTX 5060') { throw "WRONG_DEVICE:$($device.device)" }
...
[pscustomobject]@{
    status='PASS_RTX5060_PREFLIGHT'; checked_at=(Get-Date -Format o); paper=$Paper
    ...
    experiment_processes=0; authorization='DEV_PRE_V2_FIXED_SAMPLE_ONLY'
