"""Gate-locked EXP-R2 R4 runner for P1/P2 at three numerical resolutions."""
from __future__ import annotations

import argparse
import json
from pathlib import Path

from .cli import save, sha
from .pilot_runner import ROUTE_LENGTH, SPEED, run
from .r2c_analyze import audit


PARAMETERS = ("P1", "P2")
STEPS = (("2ms", 0.002), ("1ms", 0.001), ("0.5ms", 0.0005))


def load_json(path: Path):
    return json.loads(path.read_text(encoding="utf-8"))


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--out", required=True)
    parser.add_argument("--selection", required=True)
    parser.add_argument("--r3-report", required=True)
    args = parser.parse_args()
    out = Path(args.out)
    selection_path = Path(args.selection)
    r3_path = Path(args.r3_report)
    selection = load_json(selection_path)
    r3 = load_json(r3_path)
    selected = selection.get("selected_for_r3")
    if selection.get("status") != "PASS" or selected not in ("lambda1", "lambda2"):
        raise SystemExit("Valid R2c selection is required")
    if r3.get("status") != "PASS" or r3.get("scope") != "selected R2c full-route controller; R3 numerical pairing only":
        raise SystemExit("Passing R3 numerical report is required")
    lambda_internal = float(selected.removeprefix("lambda"))
    if float(r3.get("lambda_internal")) != lambda_internal:
        raise SystemExit("R2c selection and R3 lambda identities differ")
    out.mkdir(parents=True, exist_ok=False)
    registration = {
        "status": "RUNNING",
        "stage": "EXP-R2/R4",
        "selected_for_r3": selected,
        "lambda_internal": lambda_internal,
        "run_order": [f"{parameter}_{name}" for parameter in PARAMETERS for name, _ in STEPS],
        "stop_rule": "stop all remaining R4 runs after the first non-COMPLETED run",
        "selection_report_sha256": sha(selection_path),
        "r3_report_sha256": sha(r3_path),
        "runner_sha256": sha(Path(__file__).with_name("pilot_runner.py")),
        "batch_sha256": sha(__file__),
        "runs": [],
    }
    save(out, "registration.json", registration)
    save(out, "status.json", registration)
    for parameter in PARAMETERS:
        for name, max_step_s in STEPS:
            run_name = f"{parameter}_{name}"
            run_dir = out / run_name
            try:
                run(
                    run_dir,
                    parameter_id=parameter,
                    max_step_s=max_step_s,
                    lambda_internal=lambda_internal,
                    segment_s=ROUTE_LENGTH / SPEED,
                )
            except SystemExit as error:
                metrics_path = run_dir / "metrics.json"
                metrics = load_json(metrics_path) if metrics_path.is_file() else {"status": "FAILED", "reason": str(error)}
                registration["runs"].append({"name": run_name, "folder": str(run_dir), "metrics": metrics})
                registration["status"] = "FAILED"
                registration["reason"] = f"{run_name} did not complete"
                save(out, "status.json", registration)
                raise
            except Exception as error:
                registration["runs"].append(
                    {"name": run_name, "folder": str(run_dir), "exception": f"{type(error).__name__}: {error}"}
                )
                registration["status"] = "FAILED"
                registration["reason"] = f"{run_name} raised an unexpected exception"
                save(out, "status.json", registration)
                raise
            metrics = load_json(run_dir / "metrics.json")
            post_audit, _ = audit(run_name, run_dir)
            post_audit["checks"]["maximum_plant_step_identity"] = bool(
                abs(float(metrics["maximum_plant_step_s"]) - max_step_s) <= 1e-15
            )
            post_audit["checks"]["selected_lambda_identity"] = bool(
                float(metrics["lambda_internal"]) == lambda_internal
            )
            post_audit["checks"]["parameter_identity"] = metrics["parameter_id"] == parameter
            post_audit["status"] = "PASS" if all(post_audit["checks"].values()) else "FAIL"
            registration["runs"].append(
                {"name": run_name, "folder": str(run_dir), "metrics": metrics, "post_audit": post_audit}
            )
            if post_audit["status"] != "PASS":
                registration["status"] = "FAILED"
                registration["reason"] = f"{run_name} failed the independent post-run hard/evidence audit"
                save(out, "status.json", registration)
                raise SystemExit(20)
            save(out, "status.json", registration)
    registration["status"] = "COMPLETED"
    registration["reason"] = None
    save(out, "status.json", registration)
    print(json.dumps({"status": registration["status"], "runs": [item["name"] for item in registration["runs"]]}))


if __name__ == "__main__":
    main()
