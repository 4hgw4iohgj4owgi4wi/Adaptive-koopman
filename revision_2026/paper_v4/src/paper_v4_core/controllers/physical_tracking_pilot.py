...
def build_problem(z0, u_nom, refs, model, config=PilotConfig()):
    ...
    for h in range(n):
        ...
        # The decision variable is the deviation x = u - u_nom, so the realised rate is
        # rho = r_nom + D x with r_nom = u_nom[h] - u_nom[h-1].  Penalising 0.05*||rho/scale||^2
        # therefore needs the linear term +2*0.05*(D^T r_nom)/scale^2, i.e. target = +r_nom.
        # The previous code carried a minus sign here, which made the term a reward for rate
        # change proportional to r_nom and drove the realised rate toward 2*r_nom.  See
        # analysis/20260916_RATE_PENALTY_SIGN_PROBE_02 and experiment.md section 50.
