...
def aggregate_step_audit(state_endpoint: np.ndarray, control: np.ndarray, audit: dict) -> dict:
    ...
    support = np.asarray([segment["payload_support_load_n"] for segment in segments], dtype=float)
    total_normal = np.asarray([segment["vehicle_total_normal_load_n"] for segment in segments], dtype=float)
    payload_accel = np.asarray([segment["payload_accel_body_mps2"] for segment in segments], dtype=float)
    support_residual = np.asarray([segment["support_constraint_residual"] for segment in segments], dtype=float)
    support_relative = np.asarray([segment["support_constraint_relative_residual"] for segment in segments], dtype=float)
    ...
    return {
        ...
        "payload_support_load_mean_n": _weighted_mean(support, dt, duration),
        "vehicle_total_normal_load_mean_n": _weighted_mean(total_normal, dt, duration),
        "payload_accel_body_mean_mps2": _weighted_mean(payload_accel, dt, duration),
        "support_constraint_residual_max": np.max(np.abs(support_residual), axis=0),
        "support_constraint_relative_residual_max": np.max(np.abs(support_relative), axis=0),
        "minimum_support_load_n": float(np.min(support)),
        "load_transfer_enabled": bool(segments[-1]["load_transfer_enabled"]),
...
def aggregate_control_interval(step_features: list[dict], expected_duration_s: float = 0.02) -> dict:
    ...
    return {
        ...
        "payload_support_load_mean_n": weighted("payload_support_load_mean_n"),
        "vehicle_total_normal_load_mean_n": weighted("vehicle_total_normal_load_mean_n"),
        "payload_accel_body_mean_mps2": weighted("payload_accel_body_mean_mps2"),
        "support_constraint_residual_max": np.max(
            [item["support_constraint_residual_max"] for item in step_features], axis=0
        ),
        "support_constraint_relative_residual_max": np.max(
            [item["support_constraint_relative_residual_max"] for item in step_features], axis=0
        ),
        "minimum_support_load_n": float(
            min(item["minimum_support_load_n"] for item in step_features)
        ),
        "load_transfer_enabled": bool(step_features[-1]["load_transfer_enabled"]),
    }
