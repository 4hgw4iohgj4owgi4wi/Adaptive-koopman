...
class GroupedResidualEncoder(nn.Module):
    ...
    def __init__(self, residual_dim: int, branch_output: int, hidden_width: int):
        super().__init__()
        ...
        # phi_d: g0(3) + g1(16) + g2(12) + u(7) = 38
        self.branch_d = _MLP(3 + 16 + 12 + 7, int(hidden_width), self.branch_output)
        # phi_c: g2(12) + g3(16) + u(7) = 35
        self.branch_c = _MLP(12 + 16 + 7, int(hidden_width), self.branch_output)
...
class TriangularResidualKoopman(nn.Module):
    ...
    def __init__(
        ...
        if a0.shape != (47, 47) or b0.shape != (47, 7) or bias0.shape != (47,):
            raise ValueError(f"frozen S0 block shape mismatch: {a0.shape} {b0.shape} {bias0.shape}")
        ...
        self.encoder = GroupedResidualEncoder(residual_dim, branch_output, hidden_width)
        ...
        self.G = nn.Parameter(torch.zeros(self.residual_dim, 7))
    ...
    def full_k_matrix(self) -> torch.Tensor:
        """K = [[A0, E], [0, F]] block triangular (float64)."""
        a0 = self.A0.double()
        e = self.E.double()
        f = self.f_matrix().double()
        rows = a0.shape[0] + f.shape[0]
        k = torch.zeros(rows, rows, dtype=torch.float64)
        k[:47, :47] = a0
        k[:47, 47:] = e
        k[47:, 47:] = f
        return k
    ...
    def rollout(
        ...
        *,
        ...
        for step in range(int(h_max)):
            ...
            eta = eta @ self.f_matrix().to(dtype=dtype) + u_t @ self.G.to(dtype=dtype).T + self.c.to(dtype=dtype)
