...
def append_work_log(path: Path, run_id: str, stage: str, identity: dict, complete: dict, command: list[str]) -> None:
    ...
    lines = [
        ...
        "",
        "- 请求/任务编号：`koopman_dev.md / {}`".format(stage),
        "- 授权模式：执行实验",
...
def append_failure_solution(path: Path, stage: str, run_id: str, complete: dict) -> None:
    ...
    with path.open("a", encoding="utf-8") as stream:
        ...
        stream.write(f"- 已停止：{', '.join('K'+str(i) for i in range(int(stage[1:])+1, 15))}\n")
...
def main() -> None:
    ...
    parser.add_argument("--stage", choices=("K0", "K1", "K2"), required=True)
    parser.add_argument("--run-tag", default="R01")
    parser.add_argument("--resume")
    args = parser.parse_args()
    project = args.project_root.resolve()
    protocol_path = args.protocol.resolve()
    results = project / "revision_2026" / "koopman_next_results"
    runs = results / "runs"
    ...
    try:
        with contextlib.redirect_stdout(stdout_buffer), contextlib.redirect_stderr(stderr_buffer):
            if args.stage == "K0":
                ...
                stage_complete = freeze.run(project, protocol, run_dir)
            elif args.stage == "K1":
                import run_k1

                stage_complete = run_k1.run(project, protocol, run_dir)
            else:
                import run_k2

                stage_complete = run_k2.run(project, protocol, run_dir)
    ...
    complete.update(
        {
            ...
            "input_data": (
                protocol["connector_data"]
                if args.stage == "K1"
                else ("new paired K2 pilot from frozen V1-ES/R3-ES plants" if args.stage == "K2" else "frozen upstream identities")
            ),
            "read_summary": "upstream F36/F23 evidence and isolated koopman_next source",
            "modification_summary": "new isolated audit outputs; connector_r3_4 remains read-only",
        }
    )
    if args.stage == "K0":
        ...
        complete.setdefault(
            "next_action",
            "Run K1 causal schema/relative-coordinate contract only." if complete.get("passed") else "Repair K0 before K1.",
        )
    elif args.stage == "K1":
        complete["key_results"] = {
            "trajectory_count": complete.get("trajectory_count"),
            "checked_samples": complete.get("checked_sample_count"),
            "maxima": complete.get("maxima"),
        }
        complete.setdefault(
            "next_action",
            "Run the authorized K2 paired-data pilot only; K3 and training remain forbidden."
            if complete.get("passed")
            else "Repair K1 before K2.",
        )
    else:
        complete["key_results"] = {
            "trajectory_count": complete.get("trajectory_count"),
            "base_family_count": complete.get("base_family_count"),
            "coverage_counts": complete.get("coverage_counts"),
            "off_grid_trajectory_count": complete.get("off_grid_trajectory_count"),
            "replay_mismatch_count": complete.get("replay_mismatch_count"),
            "cross_split_base_family_count": complete.get("cross_split_base_family_count"),
            "projected_full_data_gib": complete.get("projected_full_data_gib"),
            "projected_full_runtime_hours": complete.get("projected_full_runtime_hours"),
        }
        complete.setdefault(
            "next_action",
            "Stop after K2; independently review coverage before separately authorizing K3."
            if complete.get("passed")
            else "Repair K2 data/coverage contracts; K3 and training remain forbidden.",
        )
    ...
    if not complete.get("passed"):
        ...
        write_deferred_limitations(results / "solutions.md", Path(complete["limitations_path"]), run_id)
