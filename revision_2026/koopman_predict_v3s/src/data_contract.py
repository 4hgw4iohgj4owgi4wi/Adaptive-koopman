...
class FrozenSequenceDataset:
    ...
    def scenario_window_counts(self) -> dict[str, int]:
        """Total 20-step cache windows per scenario over this dataset's entries
        (used for SB16 scenario-balance weights; train labels only)."""
        counts: dict[str, int] = {}
        for trajectory in self._trajectories:
            scenario = str(trajectory["entry"]["scenario"])
            counts[scenario] = counts.get(scenario, 0) + int(len(trajectory["window_starts"]))
        return counts
