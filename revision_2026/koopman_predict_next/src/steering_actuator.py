...
def step_actuator_mode(
    ...
    """Advance one of the frozen A0--A3 actuator attribution modes.

    A0 disables lag/rate/angle limits; A1 keeps lag only; A2 keeps lag and
    rate; A3 calls the unchanged parent actuator implementation.
    """

    aliases = {"instant": "A0", "dynamic": "A3"}
    selected = aliases.get(str(mode), str(mode).upper())
    if selected not in {"A0", "A1", "A2", "A3"}:
        raise ValueError(f"unknown actuator attribution mode: {mode}")
    if selected == "A3":
        output = step_actuator(delta_req_rad, delta_act_rad, step_s, config)
        return {**output, "mode": np.asarray(selected)}
    ...
    if selected == "A2":
        maximum_increment = config.rate_max_radps * step_s
        limited_increment = np.clip(
            free_increment, -maximum_increment, maximum_increment
        )
        rate_mask = np.abs(free_increment - limited_increment) > 1.0e-15
    else:
        limited_increment = free_increment
        rate_mask = np.zeros(request.shape, dtype=bool)
    actual_next = actual + limited_increment
    rate = (actual_next - actual) / step_s
    return {
        "delta_act_next_rad": actual_next,
        "delta_rate_radps": rate,
        "free_next_rad": free_next,
        "rate_limited_mask": rate_mask,
        "angle_limited_mask": np.zeros(request.shape, dtype=bool),
        "mode": np.asarray(selected),
    }
