...
from internal_force import decompose_planar_point_forces, diagonal_tension_modes, legacy_q_fr_q_lr
...
def tire_force(state: np.ndarray, acceleration: float, steering: float, support_n: float, params: VehicleParams) -> dict:
    ...
    return {"force_body_n": np.asarray([fx * scale, fy * scale]), "yaw_moment_nm": moment * scale, "utilization": utilization * scale}
...
def system_derivative(state: np.ndarray, controls: np.ndarray, params: ModelParams, law: str) -> tuple[np.ndarray, dict]:
    ...
    support = params.payload.mass_kg * params.payload.gravity_mps2 / 4.0
    vehicle_derivative = np.empty_like(vehicles)
    tires = []
    for index in range(4):
        tire = tire_force(vehicles[index], controls[index, 0], controls[index, 1], support, params.vehicle)
        tires.append(tire)
        force_body = tire["force_body_n"] + connector["force_vehicle_body_n"][index]
        _, _, yaw, vx, vy, yaw_rate = vehicles[index]
        velocity_world = rotation(yaw) @ np.asarray([vx, vy])
        vehicle_derivative[index] = [velocity_world[0], velocity_world[1], yaw_rate, force_body[0] / params.vehicle.mass_kg + yaw_rate * vy, force_body[1] / params.vehicle.mass_kg - yaw_rate * vx, (tire["yaw_moment_nm"] + connector["vehicle_moment_nm"][index]) / params.vehicle.yaw_inertia_kgm2]
    payload_rotation = rotation(payload[2])
    payload_force_world = np.sum(connector["force_payload_world_n"], axis=0)
    payload_acceleration_body = payload_rotation.T @ (payload_force_world / params.payload.mass_kg)
    vx, vy, yaw_rate = payload[3:6]
    velocity_world = payload_rotation @ payload[3:5]
    payload_derivative = np.asarray([velocity_world[0], velocity_world[1], yaw_rate, payload_acceleration_body[0] + yaw_rate * vy, payload_acceleration_body[1] - yaw_rate * vx, connector["payload_moment_total_nm"] / params.payload.yaw_inertia_kgm2])
    return np.r_[vehicle_derivative.ravel(), payload_derivative], {"connectors": connector, "tire": tires, "payload_accel_body_mps2": payload_acceleration_body}
