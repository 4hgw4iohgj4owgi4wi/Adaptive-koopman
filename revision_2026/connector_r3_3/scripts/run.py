...
def roots(project: Path) -> dict[str, Path]:
    ...
    return {
        "results": revision / "connector_r3_3_results",
        "data": revision / "connector_r3_3_data",
        "models": revision / "connector_r3_3_models",
        "mpc": revision / "connector_r3_3_mpc",
    }
...
def require_previous(results: Path, stage: str) -> None:
    previous = {"R1": "n0", "N1": "r1", "N2A": "n1", "N2": "n2a", "N3": "n2"}.get(stage)
    if not previous:
        return
...
def simulate_vehicle(name: str, law: str, label: str, outer_h: float, mode: str, max_step_s: float | None = None) -> tuple[dict, dict]:
    ...
    zone_steps = np.zeros(4, dtype=int); events = []; accepted = closure_count = 0; min_dt = float("inf")
...
def flatten_vehicle_metric(metric: dict) -> dict:
    row = {key: metric[key] for key in ("scenario", "run", "law", "mode", "outer_h_s", "max_step_s", "status", "duration_s", "runtime_s", "internal_force_peak_n", "payload_moment_peak_nm", "action_reaction_max_n", "internal_null_max_n", "accepted_steps", "min_physical_dt_s", "closure_residual_count", "finite")}
