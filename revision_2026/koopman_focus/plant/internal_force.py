...
def tension_load_proxies(force_body:np.ndarray)->dict:
    """Return longitudinal/lateral separation-load proxies for FL,FR,RL,RR."""
    f=np.asarray(force_body,float).reshape(4,2)
    front=f[0]+f[1];rear=f[2]+f[3];left=f[0]+f[2];right=f[1]+f[3]
    return {'tension_x_n':.5*max(0.,float(front[0]-rear[0])),
            'tension_y_n':.5*max(0.,float(left[1]-right[1]))}
