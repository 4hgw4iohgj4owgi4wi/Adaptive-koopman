...
- 当前框架图左下安全层示意不再使用外层截图式边框，只保留 FDI/PPC/ISS 到安全控制的原生 Visio 小元素。
- 当前控制流程图采用模块化方块和菱形判断，不再使用未解释的 ×、+、Σ、min 圆形算子。
- fault? 判断保留 yes/no 两个出口；yes 进入 FTC，no 绕过 FTC 直接进入安全滤波/PPC guard。
- cert? 判断保留 pass/fail 两个出口；pass 直接进入 u_act，fail 进入投影/收紧与货物保护回退后再到 u_act。
- 外部故障/退化只作用到通信/执行器/车辆系统，FDI 仅通过 residual/state mismatch/control response 推断故障，避免“使用真实故障标签”的误解。
- 底部反馈合并为 monitoring/logging bus，只保留少量必要反馈线；跨层线、反馈线和回退线尽量绕开模块块体，同一条折线只在末尾保留箭头。
