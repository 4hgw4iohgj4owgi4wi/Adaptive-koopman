...
- 当前框架图左下安全层示意不再使用外层截图式边框，且将原来的 safe u 改为“保护信号融合”，避免和下层 u_act 混淆。
- 当前控制流程图采用模块化方块和菱形判断，不再使用未解释的 ×、+、Σ、min 圆形算子。
- fault? 判断保留 yes/no 两个出口；yes 进入 FTC，no 绕过 FTC 直接进入安全滤波/PPC guard。
- cert? 判断保留 pass/fail 两个出口；pass 进入安全仲裁 selector，fail 进入投影/收紧与货物保护回退后再进入 selector，最后统一输出 u_act。
