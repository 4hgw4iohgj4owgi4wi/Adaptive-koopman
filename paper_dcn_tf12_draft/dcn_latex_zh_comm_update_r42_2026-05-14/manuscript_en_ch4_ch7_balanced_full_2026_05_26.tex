...
>=Latex,
proc/.style={draw=blue!65, fill=blue!7, rounded corners=2pt, align=center, minimum width=2.6cm, minimum height=0.76cm},
safe/.style={draw=red!70, fill=red!7, rounded corners=2pt, align=center, minimum width=2.6cm, minimum height=0.76cm},
comm/.style={draw=teal!70, fill=teal!7, rounded corners=2pt, align=center, minimum width=2.6cm, minimum height=0.76cm},
decide/.style={diamond, aspect=2.0, draw=orange!80, fill=orange!10, align=center, inner sep=1.5pt},
arr/.style={-Latex, line width=0.7pt, draw=#1!85},
dasharr/.style={-Latex, dashed, line width=0.65pt, draw=#1!85}
]
\node[proc] (obs) at (0,3.6) {Observation\\fusion};
\node[proc] (lift) at (3.2,3.6) {Lifted state\\$z=\Phi_\theta(\chi)$};
\node[proc] (koop) at (6.4,3.6) {IRSP Koopman\\prediction};
\node[comm] (fourws) at (0,2.0) {4WS reference\\model};
\node[comm] (pathlink) at (3.2,2.0) {Reference\\communication};
\node[comm] (quality) at (6.4,2.0) {Link-quality\\consensus};
\node[comm] (delay) at (9.6,2.0) {Delay\\compensation};
\node[proc] (remote) at (9.6,3.6) {Remote-state\\prediction};
\node[proc] (mpc) at (12.8,3.6) {MPC cost and\\constraints};
\node[safe] (fdi) at (12.8,2.0) {FDI residual\\monitoring};
\node[decide] (cert) at (16.0,2.8) {Certificate\\passes?};
\node[safe] (filter) at (16.0,0.8) {Projection and\\tightening};
\node[safe] (fallback) at (12.8,0.8) {Degraded\\fallback};
\node[proc] (cmd) at (19.2,2.8) {Command\\publication};
\node[comm] (cmdlink) at (19.2,1.2) {Command\\communication};
\node[proc] (plant) at (22.4,2.8) {Vehicle--payload\\system};
\node[proc] (logs) at (22.4,1.2) {Errors, forces,\\constraints, logs};

\draw[arr=blue] (obs) -- (lift);
\draw[arr=blue] (lift) -- (koop);
\draw[arr=blue] (koop) -- (remote);
\draw[arr=blue] (remote) -- (mpc);
\draw[arr=teal] (fourws) -- (pathlink);
\draw[arr=teal] (pathlink) -- (quality);
\draw[arr=teal] (quality) -- (delay);
\draw[arr=teal] (delay) -- (remote);
\draw[arr=red] (mpc) -- (fdi);
\draw[arr=red] (fdi) -- (cert);
\draw[arr=green] (cert) -- node[above]{yes} (cmd);
\draw[arr=red] (cert) -- node[right]{no} (filter);
\draw[arr=red] (filter) -- (fallback);
\draw[arr=red] (fallback) -- (fdi);
\draw[arr=blue] (cmd) -- (plant);
\draw[arr=teal] (cmd) -- (cmdlink);
\draw[arr=teal] (cmdlink) -- (plant);
\draw[arr=gray] (plant) -- (logs);
\draw[dasharr=gray] (logs.south) |- ++(0,-0.8) -| (obs.south);
\draw[dasharr=red] (logs.west) -- ++(-4.2,0) |- (cert.south);
\draw[dasharr=teal] (quality.south) -- ++(0,-0.85) -| (filter.south);
\node[align=left,anchor=west,font=\sffamily\scriptsize,text=gray!65!black] at (0,0.15)
{Dashed paths denote communication degradation, certificate feedback, and safety fallback channels.};
\end{tikzpicture}}
...
\begin{figure}[!tbp]
\centering
\includegraphics[width=0.98\columnwidth]{figures/fig_nrkdcc_comm_mechanism_singlecol_20260528.pdf}
\caption{Experiment 3: single-column communication-mechanism diagnostics in the 5 m/s hairpin pressure run. From top to bottom, the panels report communication delay and packet loss, quality-aware MPC signals, multi-vehicle command dispersion, and protective-mode activation.}
\label{fig:delay_mechanism_evidence}
\end{figure}

Figure~\ref{fig:delay_mechanism_evidence} shows nonzero delay, packet loss, and link-quality degradation in the pressure run: the maximum mean delay is $2.83$ steps, the packet-loss ratio reaches $0.83$, and the minimum global link quality is $0.474$. The controller responds through consensus blending, constraint tightening, command-dispersion suppression, and lateral-priority activation.
...
\footnotesize
\textbf{Interpretation:} \AKE{} and \Method{} both complete the two pressure scenarios. The main differences are tracking error and connection-constraint utilization. Force peaks are reported in parallel as force-cost audits and are interpreted together with the hairpin diagnostics.
\end{minipage}
\end{table*}
...
\begin{table*}[!t]
\centering
\caption{Experiment 7: paired $n=3$ statistics at different reference speeds under the same pressure scenario. Only the reference speed limit is changed. The table compares task completion, team lateral/longitudinal RMSE, and certificate pass ratio for \AKE{}, \MethodBase{}, and \Method{} to identify speed sensitivity.}
...
Table~\ref{tab:speed_sensitivity} reports paired $n=3$ speed sensitivity under the same pressure scenario. At 2 m/s and 5 m/s, \Method{} and \MethodBase{} complete the full path and reduce team lateral and longitudinal RMSE relative to \AKE{}. At 10 m/s, all methods still complete the task, but longitudinal RMSE reaches about $6.91$ m, indicating a separation between lateral tracking and longitudinal phase behavior. At 15 m/s, the paired protocol remains a failure case for all methods. The same-initial-condition diagnostic below is therefore reported separately to inspect whether updated low-level speed clipping, balanced progress supervision, and local lateral trimming can remove this failure mode in a representative run.

The sensitivity is consistent with increased reference speed amplifying yaw-rate demand, longitudinal phase error, and delay extrapolation. The communication-aware weights and connection constraints mainly suppress lateral and connection risks; after 10 m/s, longitudinal phase error becomes the dominant limitation.

\begin{table*}[!tbp]
\centering
\caption{Experiment 8: same-initial-condition degraded AKE-M diagnostics on sine fault/disturbance paths. The 2/5/10/15 m/s sine-reference cases use the same seed, initial error, and spatial fault trigger for \AKEdeg{} and \Method{}. The table reports team-center lateral-error RMSE, maximum pointwise gap, and pointwise win rate; the 2 m/s and 5 m/s panels are shown in Fig.~\ref{fig:nooffset_fault_low_speed_panel}, and the 10 m/s panel is retained as Supplementary Fig. S2. The maximum pointwise gap is defined as $\max_t(|e_y^{\mathrm{NR}}(t)|-|e_y^{\mathrm{AKEdeg}}(t)|)$; a nonpositive value means that \Method{} is pointwise no worse over the whole trajectory.}
\label{tab:nooffset_degraded_sine}
\scriptsize
\begin{tabular}{p{0.20\textwidth}ccccc}
\toprule
Scenario & \AKEdeg{} RMSE [m] & \Method{} RMSE [m] & $\Delta$RMSE [m] & Max gap [m] & Win rate\\
\midrule
Sine fault/disturbance, 2 m/s & 0.042905 & 0.028774 & -0.014131 & 0.010645 & 97.40\%\\
Sine fault/disturbance, 5 m/s & 0.044080 & 0.029711 & -0.014369 & 0.000000 & 100.00\%\\
Sine fault/disturbance, 10 m/s & 0.067732 & 0.053225 & -0.014507 & 0.018184 & 85.71\%\\
Sine fault/disturbance, 15 m/s & 0.158565 & 0.157407 & -0.001158 & 0.004096 & 60.70\%\\
\bottomrule
\end{tabular}
\end{table*}
...
Table~\ref{tab:nooffset_degraded_sine} and Fig.~\ref{fig:nooffset_fault_low_speed_panel} show that both methods start from zero lateral offset and use the same spatial fault trigger. At 5 m/s, \Method{} reduces team-center lateral RMSE from $0.044080$ m to $0.029711$ m, with zero maximum pointwise gap and a 100\% pointwise win rate. At 2 m/s and 10 m/s, the average RMSE also decreases, but local pointwise gaps remain. The 10 m/s diagnostic panel is retained as Supplementary Fig. S2 to document the onset of high-speed degradation. At 15 m/s, the improvement is marginal: lateral RMSE decreases only from $0.158565$ m to $0.157407$ m, and the pointwise win rate is 60.70\%. The 15 m/s result should therefore be read as a failure-mode diagnostic rather than robust performance evidence.

The pattern is consistent with network-aware prediction, lateral-priority weighting, and local-path/role scheduling suppressing lateral phase error most clearly at 5 m/s. At 2 m/s and 10 m/s, local steering phase, preview length, and longitudinal progress are not fully coordinated. At 15 m/s, low-level speed clipping and progress supervision dominate the behavior, so the diagnostic only shows that the representative run can be made complete with a very small RMSE reduction and a remaining local gap.

\subsection{High-Curvature Hairpin Comparison}

The original paired $n=3$ hairpin stress statistics show that \AKE{}, \MethodBase{}, and \Method{} complete the full path. \Method{} reduces lateral and longitudinal RMSE and improves the certificate pass ratio relative to \AKE{}, but its payload-force RMS remains higher. To avoid duplicate hairpin tables, this subsection uses the same-initial-condition three-stage table as mechanism-oriented diagnostic evidence, while the paired statistics remain background evidence.

The hairpin scenario amplifies curvature tracking, longitudinal phase progression, and four-vehicle connection geometry. Therefore, the diagnostic compares \AKEdeg{}, \MethodHC{}, and \Method{} to separate high-curvature disturbance rejection from payload-force protection.

\begin{table*}[!tbp]
\centering
\caption{Experiment 9: same-initial-condition three-stage diagnostic on the 5 m/s hairpin. The scenario uses a 5 m/s hairpin reference path, spatially triggered communication/fault disturbances, and variable 5--10 step upper-to-lower delay. The force column reports the peak of the 0.10 s centered moving-mean planar resultant force, matching the force panel in Fig.~\ref{fig:nooffset_hairpin_5mps_panel}.}
\label{tab:nooffset_degraded_hairpin}
\scriptsize
\begin{tabular}{p{0.20\textwidth}ccccc}
\toprule
Method/stage & Lat. RMSE [m] & Long. RMSE [m] & Max lat. error [m] & Max conn. util. & 0.10 s force peak [N] \\
\midrule
\AKEdeg{} & 0.298869 & 8.994818 & 0.587425 & 1.000000 & 5327.89\\
\MethodHC{} & 0.085119 & 8.071913 & 0.180423 & 0.778074 & 6240.14\\
\Method{} & 0.287473 & 12.121556 & 1.002130 & 0.307304 & 4373.90\\
\bottomrule
\end{tabular}
\end{table*}
...
These results indicate that link quality is not merely a diagnostic variable; it directly changes consensus weights, neighbor-prediction trust, and constraint tightening. Because the table already reports success, lateral/longitudinal error, connection utilization, and certificate statistics, additional bar plots are omitted from the main text to avoid redundant evidence.

\subsection{Stability-Certificate Statistics}

\begin{table}[!t]
\centering
\caption{Experiment 11: paired $n=20$ certificate statistics on the 5 m/s sine path. The table compares nominal clean, high-communication-noise, single-fault, and mixed fault/noise scenarios. It reports the Lyapunov/ISS certificate ok ratio and minimum margin to distinguish logged practical boundedness from strict global stability.}
...
Table~\ref{tab:e3_certificate} shows that high communication noise and mixed fault/noise have positive aggregate margins and certificate ok ratio $1.0$, whereas nominal clean and single fault contain negative worst-case margins. The corresponding mode-wise logs further show positive margins for the nominal and FTC-flagged steps in the pressure scenarios.

The difference arises because an aggregate minimum margin is sensitive to short numerical transients, switching instants, and local violations in non-pressure cases. The manuscript therefore uses the certificate-verified practical ISS/UUB formulation under a compact operating region, finite-density certificate failures, and fallback feasibility, rather than claiming strict global stability.
...
The results also delimit the method. Force-related quantities are treated as trade-off audits rather than primary improvement claims. In the hairpin diagnostic, the high-curvature tracking branch substantially reduces lateral error but increases the short-window force peak; the full payload-protection branch then lowers connection utilization and short-window force level at the cost of longitudinal lag and a larger local lateral-error excursion. The speed study shows that longitudinal phase error becomes dominant at higher reference speeds. The retained 10 m/s supplementary diagnostic illustrates this degradation, while the 15 m/s table entry should be read only as a failure-mode audit. Same-initial-condition single-seed diagnostics are used only to inspect mechanism-level behavior and are not merged with the paired statistics. The stability result is certificate-verified practical boundedness under the logged certificate conditions, not global asymptotic stability.
...
\todozh{Funding information, grant numbers, and author contribution statements will be added.}
