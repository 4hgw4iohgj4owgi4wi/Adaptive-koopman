...
\begin{abstract}
Purpose: This paper addresses predictive control for four-vehicle rigid-payload cooperative transport under high-curvature paths, communication delay/dropout, and actuator degradation, where team-center tracking, adjacent-distance preservation, and payload safety are coupled. Method: A network-resilient Koopman delay-compensated consensus control framework (\Method{}) is developed by integrating an input-aware stability-projected bilinear Koopman predictor, communication-quality-aware consensus MPC, upper-layer equivalent four-wheel-steering local-path generation, FDI/FTC, and certificate-triggered degraded fallback. Results: In paired $n=20$ tests, compared with a task-aligned baseline derived from the adaptive Koopman embedding method of Singh et al.\cite{singh2025adaptiveKoopman}, \Method{} reduces the lateral RMSE from 0.0453 to 0.0408 and the maximum connection utilization from 0.2999 to 0.0513 in the mixed fault/noise scenario; it also reduces the lateral RMSE from 0.0519 to 0.0424 and the maximum connection utilization from 0.3757 to 0.0452 in the high-communication-noise scenario. In the 5 m/s high-delay hairpin test, the high-curvature disturbance-rejection branch reduces the team-center lateral RMSE from 0.2989 to 0.0851, while the payload-protection branch reduces the peak resultant force from 15487.33 N to 8874.04 N. Conclusion: The results indicate that treating link quality, connection geometry, and payload-force risk as internal MPC variables can reduce connection risk and high-curvature lateral error under network degradation; when payload protection is activated, the controller trades a bounded increase in lateral error for a lower payload-force peak, revealing an objective trade-off between tracking accuracy and payload loading.
\end{abstract}
\begin{keyword}
