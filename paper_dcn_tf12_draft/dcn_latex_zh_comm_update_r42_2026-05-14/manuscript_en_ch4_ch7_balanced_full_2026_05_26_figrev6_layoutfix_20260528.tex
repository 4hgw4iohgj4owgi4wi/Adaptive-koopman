...
The system is a fixed four-vehicle cooperative-transport team carrying a rigid payload. Throughout the paper, $N=4$ is used only as compact notation for sums and wrapped neighbor indices, with vehicle index $i\in\{1,\ldots,N\}$. The global-frame position and heading of vehicle $i$ are denoted by
...
The same-initial sine diagnostic uses matched seeds, initial errors, and spatial fault triggers for \AKEdeg{} and \Method{}. Full numbers are moved to Supplementary Table S2. The key case is 5 m/s, where team-center lateral RMSE decreases from $0.044080$ m to $0.029711$ m with zero maximum pointwise gap and a 100\% pointwise win rate; 10 and 15 m/s retain longitudinal-phase limitations.
...
The paired $n=3$ hairpin statistics show full completion for \AKE{}, \MethodBase{}, and \Method{}. \Method{} lowers RMSE and improves the certificate pass ratio relative to \AKE{}, but its force RMS remains higher, so Fig.~\ref{fig:nooffset_hairpin_5mps_panel} and Supplementary Table S3 separate tracking from force protection. \MethodHC{} reduces lateral RMSE from $0.298869$ m to $0.085119$ m and maximum lateral error from $0.587425$ m to $0.180423$ m relative to \AKEdeg{}, while raising the 0.10 s force peak from $5327.89$ N to $6240.14$ N. Payload protection in \Method{} lowers maximum connection utilization from $0.778074$ to $0.307304$ and the force peak to $4373.90$ N, with force RMS $1647.34$ N. The trade-off is larger longitudinal lag and a local lateral-error excursion.
...
Experiment 11 gives paired $n=20$ certificate statistics on the 5 m/s sine path. Nominal clean, high-communication-noise, single-fault, and mixed fault/noise all have full-path rate $1.00$; their certificate-ok ratios are $0.360$, $1.000$, $0.799$, and $1.000$, and their minimum margins are $-0.0224$, $0.0049$, $-0.0111$, and $0.0033$. The full table is Supplementary Table S1. Because aggregate minimum margins are sensitive to short transients and switching instants, stability is stated as certificate-verified practical ISS/UUB in a compact operating region with finite-density failures and fallback feasibility, not as strict global stability.
