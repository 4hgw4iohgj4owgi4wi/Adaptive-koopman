...
\textbf{Algorithm 1. Online safe-command generation in \Method{}.}\par
\vspace{1pt}
\textbf{Input:} current state $x_k$; reference path $r_k$; temporary path packet $\mathcal P_{i,k}^{\mathrm{tmp}}$; neighbor packets with $q_{ij,k}$, $\tau_{ij,k}$, and $\ell_{ij,k}$; actuator-efficiency estimate $\hat\eta_k$.\par
\textbf{Parameter:} learned lift $\Phi_\theta$, IRSP-projected bilinear Koopman operators, MPC constraints, C-mode thresholds, and certificate tolerance $\epsilon_m$.\par
\textbf{Output:} safe command $u_k^{\mathrm{safe}}$.\par
\vspace{2pt}
\begin{tabularx}{\linewidth}{@{}>{\bfseries}r@{\hspace{0.55em}}>{\raggedright\arraybackslash}X@{}}
1 & Fuse received, delayed, and missing neighbor packets; update link-quality estimates, delayed-state estimates, and the temporary path packet.\\
2 & Build the online observation $\chi_k$ and lifted state $z_k=\Phi_\theta(\chi_k)$.\\
3 & Apply IRSP to the input-dependent Koopman predictor and predict $z_{k+1|k}$.\\
4 & Select the preliminary C-mode by priority $C_3>C_2>C_1>C_0$.\\
  & $C_0$: nominal tracking and connection preservation; $C_1$: high-curvature lateral-error priority; $C_2$: payload-force and connection-utilization protection; $C_3$: conservative degraded fallback.\\
5 & Tighten constraints using link quality, the prescribed-performance-control (PPC) envelope, and $\hat\eta_k$.\\
6 & Solve the communication-quality-aware consensus MPC to obtain $u_k^{\mathrm{mpc}}$.\\
7 & Apply FDI/FTC reallocation and bounded role-scheduling trim to obtain the candidate command $u_k^{\mathrm{cand}}$.\\
8 & Evaluate the Lyapunov/ISS certificate margin $m_k$ for $u_k^{\mathrm{cand}}$.\\
9 & \textbf{if} $m_k\ge -\epsilon_m$ \textbf{then} set $u_k^{\mathrm{safe}}=u_k^{\mathrm{cand}}$.\\
10 & \textbf{else} project the command, tighten constraints, or switch to the $C_3$ degraded fallback; recompute $u_k^{\mathrm{safe}}$ under the degraded safe policy.\\
11 & \textbf{end if}; publish $u_k^{\mathrm{safe}}$ and apply it to the four-vehicle rigid-payload system.\\
12 & Log states, tracking errors, connection errors, forces, constraints, C-modes, and certificate terms.\\
\end{tabularx}
\end{minipage}}
\endgroup
\caption[Online safe-command pseudocode of \Method{}]{Online safe-command pseudocode of \Method{}. The pseudocode replaces the graphical signal-flow diagram with the one-sampling-period execution logic: packet fusion and temporary-path update, lifted Koopman prediction with IRSP, C-mode arbitration, communication-aware constraint tightening, consensus MPC, FDI/FTC and role scheduling, Lyapunov/ISS certificate checking, and degraded safe fallback when the certificate margin is violated.}
