...
OUT_STEM = "fig2_nrkdcc_module_dataflow_orthogonal_v2_20260528"
...
MODULES = [
    Module("M1", "M1 Offline learning layer", 0.25, 5.57, 15.30, 1.58, "#EAF3FF", "#2F6FBB"),
    Module("M2", "M2 Upper-layer reference\nand communication", 0.25, 0.35, 3.12, 4.95, "#E8FAF5", "#15847D"),
    Module("M3", "M3 Online prediction\nand consensus-MPC", 3.55, 0.35, 4.42, 4.95, "#EEF4FA", "#4B77A8"),
    Module("M4", "M4 Safety, FDI/FTC,\nand certificate", 8.15, 0.35, 4.15, 4.95, "#FFF1F2", "#D64545"),
    Module("M5", "M5 Vehicle-payload\nenvironment and feedback", 12.48, 0.35, 3.07, 4.95, "#F3EFFB", "#7E68A4"),
]
...
NODES = {
    # M1
    "D1": n("D1", "Offline data\ngeneration", 1.65, 6.25, "offline", w=1.55),
    "D2": n("D2", "Koopman lifting\nand bilinear regression", 4.70, 6.25, "offline", w=2.35),
    "D4": n("D4", "IRSP stability\nprojection", 7.95, 6.25, "offline", w=1.90),
    "D5": n("D5", "Model package\nPhi_theta, A, B, N_l, C, bounds", 11.20, 6.25, "offline", w=2.55),
    # M2
    "R1": n("R1", "4WS local-path\npublication", 1.10, 4.05, "ref", w=1.55),
    "R3": n("R3", "Temporary path\npacket", 2.55, 4.05, "ref", w=1.30),
    "R5": n("R5", "Link-quality\nmonitor", 1.10, 2.25, "ref", w=1.55),
    "R4": n("R4", "Reference\ncommunication", 2.55, 2.25, "ref", w=1.30),
    # M3
    "C0": n("C0", "Koopman model\ninterface", 5.85, 4.55, "control", w=1.55),
    "C1": n("C1", "Observation fusion\nand lifting", 4.75, 3.70, "control", w=1.75),
    "C6": n("C6", "Koopman\nprediction", 6.95, 3.70, "control", w=1.55),
    "C3": n("C3", "Delay-compensated\nremote-state prediction", 4.75, 2.65, "control", w=1.90),
    "C5": n("C5", "Communication-quality\naware consensus", 4.75, 1.55, "control", w=1.90),
    "C7": n("C7", "Communication-quality\naware Koopman-MPC", 6.95, 2.35, "control", w=1.90),
    "C8": n("C8", "Candidate control\nsequence", 6.95, 1.05, "control", w=1.65),
    # M4
    "S1": n("S1", "FDI residual\nmonitoring", 8.90, 4.15, "safety", w=1.35),
    "S2": n("S2", "FTC command\nreallocation", 8.90, 3.05, "safety", w=1.35),
    "S7": n("S7", "Certificate\nmetrics", 10.35, 4.15, "safety", w=1.05),
    "S3": n("S3", "Certificate\npasses?", 10.35, 3.00, "safety", w=1.22, h=0.92, shape="diamond"),
    "S4": n("S4", "Projection\nand tightening", 9.05, 1.65, "safety", w=1.55),
    "S5": n("S5", "Degraded\nfallback", 10.80, 1.65, "safety", w=1.35),
    "S6": n("S6", "Safe command\noutput", 11.70, 4.15, "safety", w=1.05),
    # M5
    "E1": n("E1", "Command publication\nand communication", 14.05, 4.15, "env", w=1.70),
    "E3": n("E3", "Vehicle-payload\nsystem", 14.05, 2.85, "env", w=1.60),
    "E4": n("E4", "Feedback logs\nerrors, forces,\nconstraints", 14.05, 1.15, "env", w=1.70, h=0.82),
}
...
EDGES = [
    ...
    # M3 internal
    Edge((bottom("C0"), (5.85, 4.10), (4.75, 4.10), top("C1")), "", CONTROL),
    Edge((bottom("C0"), (5.85, 4.10), (6.95, 4.10), top("C6")), "", CONTROL),
    Edge((right("C1"), left("C6")), "", CONTROL),
    Edge((bottom("C6"), top("C7")), "", CONTROL),
    Edge((right("C3"), (6.00, 2.65), left("C7")), "", CONTROL),
    Edge((right("C5"), (6.00, 1.55), (6.00, 2.35), left("C7")), "", TEAL),
    Edge((bottom("C7"), top("C8")), "", CONTROL),
    # M4 internal
    Edge((bottom("S1"), top("S2")), "", BLACK),
    Edge((right("S2"), (9.88, 3.05), (9.88, 3.00), left("S3")), "", BLACK),
    Edge((bottom("S7"), top("S3")), "", BLACK),
    Edge((right("S3"), (11.70, 3.00), bottom("S6")), "yes", BLACK, label_offset=(0.24, 0.25)),
    Edge((bottom("S3"), (10.35, 2.20), (9.05, 2.20), top("S4")), "no", RED, label_offset=(-0.75, 0.15)),
    Edge((right("S4"), left("S5")), "", RED),
    Edge((top("S5"), (10.80, 2.20), (10.35, 2.20), bottom("S3")), "", RED, dashed=True),
    # M5 internal
    Edge((bottom("E1"), top("E3")), "", BLACK),
    Edge((bottom("E3"), top("E4")), "", ENV),
    # cross-module
    Edge((bottom("D5"), (11.20, 5.42), (5.85, 5.42), top("C0")), "Phi_theta, A, B, N_l, C, bounds", BLUE, dashed=True, label_pos=(9.25, 5.76)),
    Edge((right("R4"), (3.42, 2.25), (3.42, 2.65), left("C3")), "P_{i,k-d}^{tmp}, r_ref", TEAL, label_pos=(5.08, 3.12)),
    Edge((bottom("R4"), (2.55, 1.78), (3.52, 1.78), (3.52, 1.55), left("C5")), "q_{ij,k}, tau_{ij,k}, ell_{ij,k}", TEAL, label_pos=(4.70, 1.08)),
    Edge((right("C8"), (8.52, 1.05), (8.52, 2.75), bottom("S2")), "u_i^*, trajectory,\nmargins", CONTROL, label_pos=(8.18, 1.40)),
    Edge((right("S6"), left("E1")), "u_k^safe", BLACK, label_pos=(12.78, 4.35)),
    Edge((left("S4"), (8.02, 1.65), (8.02, 1.90), (6.00, 1.90), left("C7")), "tightened bounds,\ncontracted ref.", RED, dashed=True, label_pos=(7.00, 1.72)),
    Edge((left("E4"), (12.38, 1.15), (12.38, 3.65), (10.95, 3.65), (10.95, 4.15), right("S7")), "V_k, m_k,\nforce proxy, g(x,u)", GRAY, dashed=True, label_pos=(11.70, 3.48)),
    Edge((left("E4"), (12.65, 1.15), (12.65, 0.86), (8.18, 0.86), (8.18, 4.15), left("S1")), "y_i, yhat_i,\nr_i", GRAY, dashed=True, label_pos=(8.52, 3.62)),
    Edge((left("E4"), (12.65, 1.15), (12.65, 0.55), (4.75, 0.55), bottom("C1")), "x_{i,k+1}, e_y, e_s", GRAY, dashed=True, label_pos=(6.35, 0.78)),
]
...
def draw_matplotlib() -> None:
    ...
    fig, ax = plt.subplots(figsize=(9.6, 4.5), dpi=360)
    ax.set_xlim(0, W)
    ax.set_ylim(0, H)
    ax.axis("off")
    ...
    for e in EDGES:
        ...
        for idx in range(len(pts) - 1):
            ...
            ax.add_patch(arr)
        if e.label:
            lx, ly = e.label_pos if e.label_pos else mid(pts, e.label_offset)
            ax.text(
                lx,
                ly,
                wrap_label(e.label, 28),
                ha="center",
                va="center",
                fontsize=3.95,
                color=e.color if e.color != BLACK else "#263238",
                zorder=4,
            )
...
def shape_text(sid: int, x: float, y: float, w: float, h: float, text: str, color: str, *, font_size: float = 0.047, bold: bool = False) -> str:
    return (
        f"<Shape ID='{sid}' Type='Shape' LineStyle='3' FillStyle='3' TextStyle='3'>"
        f"{cell('PinX', x)}{cell('PinY', y)}{cell('Width', w)}{cell('Height', h)}"
        f"{cell('LocPinX', w/2, formula='Width*0.5')}{cell('LocPinY', h/2, formula='Height*0.5')}"
        ...
        f"{character(size=font_size, bold=bold, color=color)}"
        f"{rect_geometry(w, h, no_fill=1, no_line=1)}"
        f"<Text><cp IX='0'/><pp IX='0'/>{esc(text)}</Text>"
        "</Shape>"
    )
...
def page_xml() -> str:
    ...
    for e in EDGES:
        ...
        if e.label:
            lx, ly = e.label_pos if e.label_pos else mid(pts, e.label_offset)
            shapes.append(shape_text(sid, lx, ly, min(2.45, max(0.70, len(e.label) * 0.050)), 0.26 if len(e.label) < 28 else 0.42, wrap_label(e.label, 28), e.color if e.color != BLACK else "#263238", font_size=0.043))
            sid += 1
    for node in NODES.values():
        if node.kind == "diamond":
