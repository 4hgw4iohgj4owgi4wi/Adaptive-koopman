...
考虑 $N=4$ 辆车辆协同搬运刚性载荷，车辆索引为 $i\in\{1,\ldots,N\}$。第 $i$ 辆车在大地坐标系下的位置和航向为
$p_i=[X_i,Y_i]^\top$、$\psi_i$，车体系纵向速度、横向速度和横摆角速度分别为 $v_{x,i}$、$v_{y,i}$、$r_i$。控制输入向量统一定义为 $u_{i,k}^{\mathrm{ctrl}}=[a_{x,i,k},\delta_{i,k}]^\top$，其中 $a_{x,i}$ 为纵向加速度输入，$\delta_i$ 为前轮转角。车体系到大地坐标系的运动学为
\begin{equation}
\begin{aligned}
...
\end{equation}
其中 $m,I_z,l_f^{\mathrm{veh}},l_r^{\mathrm{veh}},C_f,C_r$ 分别为车辆质量、横摆转动惯量、单车质心至前/后轴距离和前/后轮侧偏刚度；$d_{x,i},d_{y,i},d_{r,i}$ 表示模型误差、载荷耦合扰动或外部扰动。为避免低速奇异，仿真中使用 $v_{x,i}\leftarrow\max(v_{x,i},v_{x,\min})$，当前代码取 $v_{x,\min}=0.5$。
...
质心至前轴距离 & $l_f^{\mathrm{veh}}$ [m] & 1.2 & 1.2\\
质心至后轴距离 & $l_r^{\mathrm{veh}}$ [m] & 1.6 & 1.6\\
...
设参考路径弧长为 $s$，曲率为 $\kappa_r(s)$，路径切向航向为 $\psi_r(s)$。车辆投影到路径坐标系后，横向误差为 $e_{y,i}$，航向误差为 $e_{\psi,i}=\psi_i-\psi_r(s_i)$。在 $| \kappa_r(s_i)e_{y,i}|<1$ 的工作域内，标准 Frenet 误差动力学为
\begin{equation}
...
载荷中心位姿为 $\xi_L=[X_L,Y_L,\psi_L]^\top$，其平面位置为 $p_L=[X_L,Y_L]^\top$。第 $i$ 个连接点在载荷坐标系中的固定向量为 $b_i$，对应大地坐标位置为
\begin{equation}
...
\end{equation}
若采用等效弹簧阻尼连接，且 $K_c,D_c\in\R^{2\times2}$ 为半正定连接刚度和阻尼矩阵，则载荷侧连接力代理可表示为
\begin{equation}
F_{c,i}=K_c e_{c,i}+D_c\dot e_{c,i},
\label{eq:connection_force}
\end{equation}
并通过 $\sum_i F_{c,i}$ 与 $\sum_i (R(\psi_L)b_i)\times F_{c,i}$ 作用到载荷平动和转动，其中二维叉乘定义为 $a\times b=a_xb_y-a_yb_x$。本文在控制层不声称连接力峰值单调降低，而是将 $\max_i\norm{F_{c,i}}$、均方根（root mean square, RMS）和方向分量作为安全代价监测指标。

\subsection{通信图与退化模型}

车辆通信拓扑扩展为五节点图 $\mathcal G_k=(\mathcal V\cup\{0\},\mathcal E_k)$，其中 $0$ 表示上层协同路径规划器，$\mathcal V=\{1,2,3,4\}$ 表示四辆执行车辆。车辆层采用环形邻接通信，每辆车 $i$ 接收前后相邻车辆 $\mathcal N_i=\{i-1,i+1\}$ 的相对位姿和相对距离，邻居索引按模 $N$ 环绕。上层节点根据载荷中心状态、参考路径曲率和团队横向误差计算短期临时期望路径，并把该路径下发到四辆车。令 $s_k$ 为载荷/团队中心参考进度，$e_{y,L,k}$ 为载荷/团队中心投影到参考路径后的横向误差。通信退化同时作用于三类量：
\begin{equation}
\begin{aligned}
...
\end{equation}
其中 $d_{ij,k}=\norm{p_{i,k}-p_{j,k}}$ 为相邻车辆距离，$\mathcal P_{i,k}^{\mathrm{tmp}}$ 为上层下发给车辆 $i$ 的临时期望路径包；$\tau,\beta,\epsilon$ 分别表示时延、偏置和噪声。边 $(j,i)$ 的链路质量为 $q_{ij,k}\in[0,1]$，聚合网络质量记为 $q_k^{\mathrm{net}}$。丢包指示为 $\ell_{ij,k}\in\{0,1\}$，其中 $\ell_{ij,k}=1$ 表示丢包，$\ell_{ij,k}=0$ 表示成功接收。当邻居状态可用时，控制器接收延迟状态；当时延或丢包发生时，使用 Koopman/运动学预测器进行外推：
\begin{equation}
...
\end{equation}
这里 $\Pi_x$ 为 lifted state 到物理状态的投影，$\hat z_{j,k|k-\tau^x_{ij,k}}$ 表示由延迟接收时刻预测到当前时刻的 lifted state。相邻距离保持项定义为
\begin{equation}
...
设等效 4WS 平台前后轴距为 $l_f^{\mathrm{4ws}},l_r^{\mathrm{4ws}}$，$L^{\mathrm{4ws}}=l_f^{\mathrm{4ws}}+l_r^{\mathrm{4ws}}$。四轮转向等效侧偏角与曲率写为
\begin{equation}
\begin{aligned}
\beta^{\mathrm{4ws}} &=
...
\end{equation}
上层求解器以参考曲率 $\kappa_r(s)$ 为目标，并对侧偏、前后转角关系和转角幅值加权，其中 $w_\kappa,w_\beta,w_\rho,w_\delta\ge0$：
\begin{equation}
\begin{aligned}
...
对第 $i$ 辆车，设其在载荷坐标系中的连接点为 $b_i=[b_{x,i},b_{y,i}]^\top$，并令 $\psi_{L,k}^{\mathrm{ref}}=\psi_r(s_k)$。由式 \eqref{eq:fourws_beta_curvature} 得到的等效刚体瞬时速度方向可写为
\begin{equation}
\psi_{i,k}^{\mathrm{up}}=
\psi_{L,k}^{\mathrm{ref}}+
...
\end{equation}
上层再沿载荷参考路径 $p_L^{\mathrm{ref}}(s)$ 向前预瞄 $\ell_p$，构造车辆 $i$ 的临时期望路径包
\begin{equation}
...
\end{equation}
其中 $\alpha_p$ 为局部路径混合系数。通信扰动既可作用于邻接距离和团队横向误差，也可作用于 $\mathcal P_{i,k}^{\mathrm{tmp}}$ 的下发链路；因此本文把上层--下层路径传输作为五节点通信图中的独立边处理。实验中 5 m/s 回头弯采用 $\ell_p=0.45$ m、$\rho_r=0.2$、局部路径横向幅值上限 $0.04$ m 和 $s=53.5$--$61.5$ m 的启用窗口，用于只在最需要的高曲率出口段注入等效四轮转向先验。

\subsection{离线数据生成}

离线数据由式 \eqref{eq:world_kinematics}--\eqref{eq:connection_force} 的仿真模型生成，覆盖 nominal clean、high communication noise、single fault 和 mixed fault/noise 四类工况。训练数据采用 $\Delta t=0.02$ s，单车原始状态维数为 $6$，输入向量为 $u_{i,k}^{\mathrm{ctrl}}=[a_{x,i,k},\delta_{i,k}]^\top$，输入维数 $n_u=2$；参考速度从 $2.2$--$4.0$ m/s 分层采样，初始路径进度随机平移 $0$--$80$ m，横向误差初值在 $[-2,2]$ m 内扰动。每个 batch 生成 36 条轨迹和约 1400 个 snapshot，并按 80/20 划分训练集与验证集。每个 run 同步记录车辆状态、载荷中心、连接误差、控制输入、通信质量、时延/丢包、故障效率和证书项。为避免训练数据只覆盖平稳轨迹，输入激励包含小幅随机转角、纵向加速度扰动、曲率变化和通信质量扰动。

\subsection{双线性 Koopman 提升}

令 $x_{i,k}\in\R^6$ 为第 $i$ 车原始物理状态。学习和控制使用增强观测 $\chi_k\in\R^{n_\chi}$，其中包含车辆状态、载荷/团队中心误差、连接几何、聚合网络质量 $q_k^{\mathrm{net}}$、故障效率估计和路径上下文变量。令 $u_k^{\mathrm{ctrl}}\in\R^{n_u}$ 为进入 Koopman 预测器的控制输入向量。Koopman lifting 定义为
\begin{equation}
...
\end{equation}
其中 $A,N_\ell\in\R^{n_z\times n_z}$、$B\in\R^{n_z\times n_u}$ 由离线数据拟合，$w_k$ 为残差。该结构比纯线性 Koopman 模型更适合描述转角、纵向加速度与车辆侧向/载荷误差之间的乘性耦合。

给定数据集 $\mathcal D=\{z_k,u_k^{\mathrm{ctrl}},z_{k+1}\}_{k=1}^{M-1}$，定义回归矩阵
\begin{equation}
...
\end{equation}
令 $Z_+=[z_2,\ldots,z_M]$、$\Xi=[\Xi_1,\ldots,\Xi_{M-1}]$。参数矩阵 $\Theta=[A\;B\;N_1\;\cdots N_{n_u}]$ 由岭回归求解：
\begin{equation}
\Theta^\star=Z_+\Xi^\top(\Xi\Xi^\top+\lambda I)^{-1}.
\label{eq:ridge_fit}
\end{equation}
其中 $\lambda>0$ 为岭回归正则系数，$I$ 为与 $\Xi\Xi^\top$ 维度匹配的单位阵，用于抑制小样本、多重共线和噪声导致的参数放大。在线阶段采用滑动窗口残差更新，只允许在输入感知稳定投影和约束边界内微调 $\Theta$，避免自适应过程破坏闭环可行性。

\subsection{输入感知稳定投影与残差边界}

仅对 Koopman 主矩阵 $A$ 做谱投影不能覆盖双线性输入项。由式 \eqref{eq:bilinear_koopman} 可知，在给定输入 $u_k^{\mathrm{ctrl}}$ 下真正进入预测窗口的等效 lifted 矩阵为
\begin{equation}
A_{\mathrm{eff}}(u_k^{\mathrm{ctrl}})=A+\sum_{\ell=1}^{n_u}u_{k,\ell}^{\mathrm{ctrl}}N_\ell .
\label{eq:effective_bilinear_matrix}
\end{equation}
因此本文将稳定化步骤改为输入感知鲁棒谱投影（input-aware robust spectral projection, IRSP）。实现上先用谱投影算子 $\Pi_{r_A}(\cdot)$ 把主矩阵投影到较小漂移半径 $0<r_A<r_u$，再对双线性输入矩阵统一缩放：
\begin{equation}
...
MPC 在每个采样时刻求解 horizon $H\in\mathbb Z_{>0}$ 内的输入序列。令 $\xi_{v,i,k}=[e_{y,i,k},e_{\psi,i,k}]^\top$，并取 $Q_L,P_L,Q_v,Q_c,Q_e,R_u,R_\Delta\succeq0$、$w_d\ge0$。基础代价函数为
\begin{equation}
\begin{aligned}
J=&\sum_{h=0}^{H-1}
\Big[
...
\label{eq:mpc_cost}
\end{equation}
其中 $e_L=[X_L-X_L^{\mathrm{ref}},Y_L-Y_L^{\mathrm{ref}},\psi_L-\psi_L^{\mathrm{ref}}]^\top$，$e^d_{ij}$ 表示由式 \eqref{eq:adjacent_distance_error} 定义的相邻距离误差，$\Delta u_{k+h}^{\mathrm{ctrl}}=u_{k+h}^{\mathrm{ctrl}}-u_{k+h-1}^{\mathrm{ctrl}}$。通信权重定义为
\begin{equation}
...
\end{equation}
当 $q_k^{\mathrm{net}}$ 下降时，$e_{c,\max}(q_k^{\mathrm{net}})$ 与输入边界按比例收缩；当链路质量低于阈值时，degraded fallback 降低不可靠邻居权重并优先满足连接误差与输入约束。
...
\begin{equation}
Q_v^{\mathrm{hc}}=\mathrm{diag}(\eta_y,\eta_\psi)Q_v,\qquad
R_u^{\mathrm{hc}}=\mathrm{diag}(1,\eta_\delta)R_u ,
\label{eq:high_curvature_lateral_priority}
\end{equation}
其中 $\eta_y>1$、$\eta_\psi>1$、$0<\eta_\delta<1$，且 $R_u$ 的输入顺序与 $u_i^{\mathrm{ctrl}}=[a_{x,i},\delta_i]^\top$ 一致。同时收紧横向预设性能控制（prescribed performance control, PPC）包络 $\rho_y$ 并减小转角平滑系数，使控制器在入弯和弯中优先消除 $e_y$ 与 $e_\psi$，再由一致性项和连接约束抑制单车过度纠偏。5 m/s 回头弯中的 \MethodHC{} 采用 $\eta_y=1.35$、$\eta_\psi=1.20$、$\eta_\delta=0.82$ 的保守设置，并在 $s=30$--$50$ m 内按空间位置切换预减速、横向优先和进度恢复权重。该模式优先证明大曲率抗干扰能力，但不把载荷合力作为主优化目标，因此需要额外审计受力代价。

第二层为货物保护切换器。该分支由曲率窗口激活，并在窗口内增强连接、输入一致性和受力代理惩罚，使控制器从“轨迹误差优先”平滑切换为“误差--连接--受力三目标折中”。切换强度定义为
\begin{equation}
...
\end{equation}
其中 $\lambda_c,\lambda_u,\lambda_F\ge0$，$\bar u_k^{\mathrm{ctrl}}=\frac{1}{N}\sum_i u_{i,k}^{\mathrm{ctrl}}$ 为四车平均控制输入，$\hat F_{L,k}^{xy}$ 为由连接误差和阻尼项得到的平面载荷合力代理。该切换器不追求在所有时刻继续压低横向误差，而是在保证路径误差仍可接受的前提下降低合力峰值和连接激励；当曲率降低到 $\kappa_{\mathrm{off}}$ 附近时，$\sigma_k$ 自动淡出，避免尾段长期牺牲跟踪性能。
...
\end{equation}
FDI 监测器基于预测残差和控制响应估计 $\hat\eta_{i,k}$。设 $\varepsilon_{i,k}^{\mathrm{pred}}$ 为车辆 $i$ 的一步预测残差，在线监测量采用指数加权移动平均（exponentially weighted moving average, EWMA）与累计和（cumulative sum, CUSUM）组合：
\begin{equation}
