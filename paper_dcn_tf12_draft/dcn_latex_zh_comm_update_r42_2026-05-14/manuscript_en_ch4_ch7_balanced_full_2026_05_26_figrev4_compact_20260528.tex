...
The same-initial-condition sine diagnostic uses identical seeds, initial errors, and spatial fault triggers for \AKEdeg{} and \Method{} at 2/5/10/15 m/s. To reduce the table wall in the main text, the full numeric table is moved to Supplementary Table S2. The key entries are retained here: at 5 m/s, the team-center lateral RMSE decreases from $0.044080$ m to $0.029711$ m, with zero maximum pointwise gap and a 100\% pointwise win rate; at 10 m/s, the RMSE still decreases, but a local pointwise gap remains; at 15 m/s, the improvement becomes marginal.
...
Figure~\ref{fig:nooffset_fault_low_speed_panel} shows that both methods start from zero lateral offset and use the same spatial fault trigger. The 10 m/s diagnostic panel is retained as Supplementary Fig. S2 to document the onset of high-speed degradation. The 15 m/s result should be read as a failure-mode diagnostic rather than robust performance evidence.
...
\begin{figure*}[!tbp]
\centering
\includegraphics[width=0.84\textwidth]{figures/fig_nrkdcc_nooffset_hairpin_5mps_panel_force_20260528.pdf}
...
\begin{table}[H]
\centering
\caption{Experiment 10: compact core-module ablation on the sine path.}
