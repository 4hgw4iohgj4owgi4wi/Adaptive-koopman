...
\begin{abstract}
四车刚性载荷协同运输在高曲率路径、通信时延/丢包和执行器退化共同作用下，容易出现团队中心跟踪、邻接距离保持与载荷安全之间的冲突。为解决网络退化条件下的预测控制问题，本文提出网络韧性 Koopman-时延一致性协同运输控制（\Method{}），将输入感知稳定投影双线性 Koopman 预测器、通信质量感知一致性 MPC、上层等效四轮转向局部路径发布、FDI/FTC 和证书触发退化回退统一到同一闭环。实验表明，相对来源于 Singh 等自适应 Koopman 嵌入方法\cite{singh2025adaptiveKoopman}的任务对齐机制基线，\Method{} 在 mixed fault/noise 场景中将横向 RMSE 从 0.0453 降至 0.0408、连接最大利用率从 0.2999 降至 0.0513；在 high-communication-noise 场景中将横向 RMSE 从 0.0519 降至 0.0424、连接最大利用率从 0.3757 降至 0.0452；5 m/s 高延迟回头弯中，高曲率抗扰分支将团队中心横向 RMSE 从 0.2989 降至 0.0851，货物保护分支将合力峰值从 15487.33 N 降至 8874.04 N。结果表明，双线性 Koopman 预测模型只有与链路质量、连接几何和载荷风险共同嵌入 MPC 闭环时，才能把数据驱动预测优势转化为网络退化协同运输中的约束控制能力；货物保护模式会以可接受的横向误差回升换取载荷力峰值降低，体现 Koopman-MPC 闭环中跟踪精度与载荷受力之间的客观权衡。
\end{abstract}
\begin{keyword}
