...
\begin{figure*}[t]
\centering
\includegraphics[width=0.98\textwidth]{figures/fig2_nrkdcc_module_dataflow_pub_20260528.pdf}
\caption[Module-level online signal flow of \Method{}]{Module-level online signal flow of \Method{} with labeled data interfaces. The diagram separates offline Koopman learning, reference and communication processing, online prediction and MPC, safety/certificate handling, and environment feedback. Solid arrows denote forward data and command flow within one sampling period; dashed arrows denote offline model loading or closed-loop feedback; red arrows denote certificate-triggered projection, tightening, and fallback paths. If the certificate fails, the no branch enters projection/tightening and fallback before returning to the certificate check; feedback logs provide the measured state, tracking errors, force/constraint terms, and certificate quantities for the next sampling period.}
