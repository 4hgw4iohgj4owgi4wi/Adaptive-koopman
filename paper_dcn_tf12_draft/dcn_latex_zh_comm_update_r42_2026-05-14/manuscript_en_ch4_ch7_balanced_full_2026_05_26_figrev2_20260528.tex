...
\fancyhead[ER]{\em \footnotesize Anonymous Author(s)}
...
\caption{Experiment 3: single-column communication-mechanism diagnostics in the 5 m/s hairpin communication-stress run. From top to bottom, the panels report communication delay and packet loss, quality-aware MPC signals, multi-vehicle command dispersion, and protective-mode activation. Mean delay is reported in control steps, whereas packet loss is reported as a ratio.}
...
\begin{table*}[!t]
\centering
\caption{Experiment 6: paired $n=20$ main comparison under mixed fault/noise and high-communication-noise scenarios. The table compares the proposed method, task-aligned Koopman baselines, and a low-order auxiliary baseline in terms of success rate, full-path success, tracking error, connection-constraint utilization, and raw logged force-cost peak. Table~\ref{tab:nooffset_degraded_hairpin} separately reports a 0.10~s moving-mean force peak for the hairpin diagnostic.}
\label{tab:e0_main}
\scriptsize
