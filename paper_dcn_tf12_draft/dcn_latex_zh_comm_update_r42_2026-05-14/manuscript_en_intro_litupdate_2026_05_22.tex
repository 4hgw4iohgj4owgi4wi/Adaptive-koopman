...
Cooperative transport and networked control studies have covered formation control, distributed optimization, communication topology, vehicular ad hoc networks, vehicular edge computing, and CAV/DMPC control under communication degradation\cite{an2023multiRobotCommTransport,tan2022mlVehicularDcn,zhao2025blockchainVanetDcn,wu2025drlVecDcn,bian2025markovPacketLossDMPC,dosDMPC2024isa,robustCommunicationLoss2025trb}. Most existing work, however, focuses on spacing regulation, string stability, or single-vehicle trajectory tracking. It does not jointly incorporate team-center error, vehicle-level short-horizon paths, connection utilization, payload force, communication quality, and actuator faults into an auditable predictive-control loop for a four-vehicle rigid-payload system. This paper proposes a network-resilient Koopman delay-compensated consensus control framework, termed \Method{}, for high-curvature and communication-degraded cooperative transport. High-speed simulations are included only as sensitivity and stress tests, not as the main performance claim.

The main contributions are summarized as follows.
\begin{enumerate}
\item \textbf{Koopman learning:} An input-aware, stability-projected bilinear Koopman predictor is developed for four-vehicle rigid-payload transport. Vehicle states, team-center errors, connection geometry, and communication/fault indicators are included in the lifted observation, while bilinear terms capture the coupling between control inputs and lifted states. The stability projection further constrains $A_{\mathrm{eff}}(u)$ to reduce prediction-drift risk under high curvature and degraded communication.
\item \textbf{MPC control:} A risk-aware mode-reconfigurable Koopman-MPC scheme is proposed, rather than a set of unrelated MPC controllers. Under the same bilinear prediction model, the controller reconfigures weights, constraints, consensus compensation, and fallback candidates according to communication quality, path curvature, connection utilization, payload-force risk, and certificate margins. This changes the control priority among nominal tracking, network-resilient coordination, high-curvature payload protection, and safety fallback.
\item \textbf{Experimental validation:} A multi-level evidence chain is constructed from Koopman learning to closed-loop control, including training loss, short-window prediction, dimension ablation, communication/delay diagnostics, FDI/FTC timelines, baseline comparison, speed sensitivity, high-curvature hairpin tests, connection-risk audit, and payload-force audit. The experiments focus on team-center tracking, connection constraints, and payload safety under high curvature and communication degradation.
\item \textbf{Stability analysis:} A Lyapunov/input-to-state-stability (ISS) practical stability argument is provided for the mode-reconfigurable control structure. Koopman residuals, communication disturbances, actuator degradation, stability projection, constraint tightening, and safety fallback are incorporated into a unified boundedness analysis, leading to practical input-to-state boundedness/uniform ultimate boundedness (UUB), instead of overclaiming asymptotic convergence under persistent network faults.
\end{enumerate}
