...
\introproblem{问题P1}{四车载荷协同运输不能只用单车轨迹误差评价；需要同时覆盖载荷中心误差、连接约束、车辆输入和载荷受力等运输安全变量。}

Koopman 学习为非线性车辆控制提供了可优化预测模型。有限维 Koopman 表示与 Koopman 模型预测控制（model predictive control, MPC）的基础工作表明，可以把非线性动力学嵌入 lifted space 后用线性预测器服务于约束优化控制\cite{brunton2016koopmanControl,korda2018koopmanMpc}。近年 Koopman-MPC 已用于自动驾驶车辆横向控制、随机 MPC 和深度 Koopman 表示学习，例如基于 Koopman 的随机模型预测控制（Koopman-based stochastic model predictive control, K-SMPC）将 Koopman 预测误差纳入随机 MPC，深度 Koopman 扩张状态观测器（extended state observer, ESO）方法将观测器用于补偿车辆模型残差，核 Koopman 误差界研究也强调有限数据预测器需要误差边界和保守保护\cite{kim2025ksmpc,chen2024esoDeepKoopman,philipp2023koopmanErrorBounds,cibulka2021koopmanVehicleMpc}。与本文最直接相关的是 Singh 等提出的自适应 Koopman 嵌入（Adaptive Koopman Embedding, AKE），该方法先离线学习名义 Koopman lifting 和预测矩阵，再用在线自适应模块修正预测 lifted state 与观测 lifted state 的偏差，从而增强非线性系统在噪声、扰动和参数变化下的 MPC 鲁棒性\cite{singh2025adaptiveKoopman}。本文后续记为 \AKE{} 的机制基线即来源于该 AKE 思想，但将其任务对齐到四车刚性载荷运输场景。上述研究说明 Koopman 模型适合为 MPC 提供局部线性或近线性的预测结构，但现有 AKE/Koopman-MPC 主要面向单体或一般非线性系统控制，不直接处理四车刚性载荷运输中的邻接连接误差、载荷受力、上/下层路径通信和网络退化耦合。
\introproblem{问题P2}{现有 AKE/Koopman-MPC 缺少面向四车刚性载荷、连接误差、受力和网络上下文的联合预测模型与稳定化审计。}

多机器人协同运输研究已经发展出 formation control、分布式优化、非完整约束处理和学习型协作策略。近期综述指出，协同搬运不仅涉及协调和任务分配，还依赖通信拓扑、验证平台和中间件支撑\cite{an2023multiRobotCommTransport}；差速移动机器人协同搬运、六自由度连接器搬运、异构多 formation 运输规划、嵌入式 formation 搬运和多目标强化学习运输等工作则分别从控制结构、连接机构、任务规划、平台验证和任务决策层面推进了该方向\cite{cooperativeTransport2024ras,gong2023sixDofTransport,zhang2024heterogeneousTransport,embeddedFormationTransport2024cep,multiObjectiveTransport2024mechatronics,intelligentCollaborativeTransport2024}。然而，很多协同运输工作将通信视为理想条件或实现细节，缺少对丢包、时延和链路质量如何影响载荷连接约束风险与受力审计的闭环分析。
\introproblem{问题P3}{协同运输研究常把通信作为理想条件或实现细节，缺少丢包、时延和链路质量对连接风险与载荷受力的闭环证据。}

网络化 MPC、联网自动驾驶车辆（connected and automated vehicles, CAV）和分布式模型预测控制（distributed model predictive control, DMPC）文献近三年开始系统研究通信退化。Digital Communications and Networks 中关于 vehicular networking 的综述指出，高动态车辆网络需要同时面对异构链路、服务质量（quality of service, QoS）差异和低时延要求；该刊近期车载自组织网络（vehicular ad hoc network, VANET）/车载边缘计算研究也进一步强调认证、边缘资源和通信时延对车联网稳定服务的影响\cite{tan2022mlVehicularDcn,zhao2025blockchainVanetDcn,wu2025drlVecDcn}。在控制层，已有工作考虑 Markov 丢包、拒绝服务（denial-of-service, DoS）攻击、通信中断、随机时延、耦合安全约束和网络延迟缓解策略\cite{bian2025markovPacketLossDMPC,dosDMPC2024isa,robustCommunicationLoss2025trb,bai2023robustLongitudinalDMPC,bao2024robustUkfMpc,networkLatencyCav2024sensors,dai2024networkedPredictiveControl}。这些方法多服务于纵向车队、车距调节或队列稳定性。协同运输的关键安全变量并不是车距串稳定性，而是载荷中心误差、角点连接误差、车辆输入约束和载荷受力。因此，通信退化需要进入协同运输 MPC 的一致性权重、邻居预测、约束收缩和 fallback 触发逻辑。
\introproblem{问题P4}{已有 CAV/DMPC 网络退化控制多面向车距或串稳定；协同运输需要把通信退化显式映射到一致性权重、邻居预测、约束收缩和 fallback。}

容错与鲁棒 MPC 方面，经典约束 MPC 和有界扰动鲁棒 MPC 给出了递归可行、稳定性和扰动不变集分析基础\cite{mayne2000constrainedMpc,mayne2005robustMpc}，tube/convex robust DMPC、learning-supported MPC 和网络化预测控制进一步为处理模型不确定性与外部扰动提供了工程路径\cite{convexRobustDMPC2025ejc,gasparino2023nnMpcUncertainty,dai2024networkedPredictiveControl}。车辆执行器故障研究中，线控转向或双电机转向系统的故障检测与隔离（fault detection and isolation, FDI）/故障容错控制（fault-tolerant control, FTC）已被用于保持车辆稳定和轨迹跟踪能力\cite{liu2024adaptiveFtcSbw,xu2024fdFtcSteering}。本文将 FDI/FTC 作为闭环保护层，与通信质量估计和连接约束收缩共同触发保守可行控制，使故障检测、控制重分配和证书监测服务于四车载荷运输的连接安全。
\introproblem{问题P5}{执行器故障保护不能只验证单车可控性；还需要与通信质量、连接约束、货物保护和 Lyapunov/ISS 证书共同闭合。}
\reviewblock{FDI/FTC 近三年直接对标文献和多 seed 检测延迟统计仍需扩充；当前主文中应把它写作闭环保护增强模块，而不写作独立超越 FTC 文献的理论贡献。}

基于上述研究空白，本文提出 \MethodCN{}（\Method{}）框架。该命名对应本文的核心对象：网络韧性、Koopman 双线性预测、时延补偿一致性 MPC、上层四轮转向局部路径发布和协同运输闭环。本文基线来源于 Singh 等在 The International Journal of Robotics Research 发表的 AKE 方法\cite{singh2025adaptiveKoopman}：该机制基线保留离线 Koopman 表示学习与在线模型修正的核心思想，并在本文场景中实现为任务对齐的 \AKE{} 机制基线。与该来源方法相比，本文在同一四车载荷运输任务中进一步将通信质量、载荷连接约束风险、上/下层路径链路和故障降级共同放入可审计闭环。
\reviewblock{当前主基线是任务对齐的 \AKE{} 机制基线，不是上传原文完整自适应 AKE 版本（AKE-A）的完整复现；投稿前若要强化顶刊说服力，需要补严格 AKE-A 复现或强物理/DMPC 基线。}

本文贡献如下。

\begin{enumerate}
\item \textbf{Koopman 网络方面：} 在 Koopman 车辆 MPC 研究基础上，提出面向四车刚性载荷运输的输入感知稳定投影双线性 Koopman 预测器。该网络使用载荷中心、车辆局部误差、连接几何和通信/故障上下文构造 24 维观测与 31 维升维状态，保留 lifted dynamics 中控制输入对状态演化的乘性影响，并对 $A_{\mathrm{eff}}(u)$ 而非仅对主矩阵 $A$ 做鲁棒谱半径审计和缩放，从而限制输入相关长时域预测漂移。
\item \textbf{MPC 控制方面：} 在 CAV/DMPC 通信退化和协同运输分层控制研究基础上，提出通信质量感知的时延补偿一致性 Koopman-MPC。其机制是将丢包、时延和链路质量映射为邻居状态外推、一致性权重、输入约束收缩和 degraded fallback，同时通过上层等效四轮转向局部路径发布、FDI/FTC 控制重分配、角色调度和货物保护切换器，将团队横向误差、连接约束和载荷受力纳入同一个闭环控制框架。
\item \textbf{实验验证方面：} 建立从模型学习到闭环控制的多层证据链，包括 Koopman 训练损失、短窗口预测、维度消融、通信/时延诊断、FDI/FTC 时间线、主基线对比、速度敏感性、高曲率回头弯、连接风险与载荷受力审计。该实验设计不仅比较 \Method{} 与来源于 Singh 等 AKE 思想的任务对齐 \AKE{} 基线，还通过 \MethodBase{}、\MethodHC{}、\ZOHcons{} 和模块消融解释性能改善来自哪些具体模块。
\item \textbf{稳定性证明方面：} 给出与上述切换控制结构一致的 Lyapunov/输入到状态稳定（input-to-state stability, ISS）实践稳定性证明。证明将 Koopman 残差、通信退化、执行器退化、约束收缩、FDI/FTC、货物保护和 fallback 切换共同纳入增强 Lyapunov 函数，在有界工作域、有限切换密度和保护分支可行假设下得到输入到状态实践有界/一致最终有界（uniform ultimate boundedness, UUB）结论，并用分模式证书统计对理论边界进行数值核验。
\end{enumerate}
