...
本文贡献如下。
\begin{enumerate}
\item \textbf{用输入感知稳定投影双线性 Koopman 学习完善网络退化协同运输的预测模型。} 该模型把车辆状态、团队中心误差、连接几何以及通信/故障指标共同纳入 lifted observation，并通过双线性项显式刻画转角、纵向加速度与 lifted state 的乘性耦合。进一步地，稳定性投影直接约束控制器实际使用的 $A_{\mathrm{eff}}(u)$，从模型层降低高曲率和通信退化下的预测漂移风险。
\item \textbf{用风险感知模式重构 Koopman-MPC 解决通信干扰、高曲率路径和载荷保护之间的控制优先级冲突。} 在同一个双线性预测模型下，控制器根据通信质量、路径曲率、连接利用率、载荷受力风险和证书裕度重构权重、约束、一致性补偿与 fallback 候选；同时结合上层等效四轮转向局部路径发布、邻接距离保持和 FDI/FTC，使标称跟踪、网络韧性、高曲率货物保护和安全回退在统一闭环内切换。
\item \textbf{用 Lyapunov/ISS 证明和任务对齐实验验证闭环安全边界与性能来源。} 理论上，证明将 Koopman 残差、通信扰动、执行器退化、稳定性投影、约束收缩和安全回退纳入统一有界性论证，得到输入到状态实践有界/一致最终有界（uniform ultimate boundedness, UUB）结论。实验上，训练损失、短窗口预测、维度消融、通信/时延诊断、FDI/FTC 时间线、主基线对比、速度敏感性和 5 m/s 高曲率回头弯共同支撑模型学习、通信韧性和载荷保护模块的作用边界。
\end{enumerate}
