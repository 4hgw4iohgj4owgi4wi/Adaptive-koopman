...
协同运输研究已经覆盖 formation control、分布式优化、非完整约束、连接机构设计和学习型任务决策。近期工作分别从多机器人通信与协同搬运综述、差速移动机器人协同运输、六自由度连接器搬运、异构多 formation 运输规划、嵌入式 formation 搬运和多目标强化学习运输等角度推进了该方向\cite{an2023multiRobotCommTransport,cooperativeTransport2024ras,gong2023sixDofTransport,zhang2024heterogeneousTransport,embeddedFormationTransport2024cep,multiObjectiveTransport2024mechatronics,intelligentCollaborativeTransport2024}。与此同时，网络化控制与车联网研究已经讨论 VANET、车载边缘计算、QoS 约束、Markov 丢包、DoS 攻击、通信中断、随机时延和网络延迟补偿等问题\cite{tan2022mlVehicularDcn,zhao2025blockchainVanetDcn,wu2025drlVecDcn,bian2025markovPacketLossDMPC,dosDMPC2024isa,robustCommunicationLoss2025trb,bai2023robustLongitudinalDMPC,bao2024robustUkfMpc,networkLatencyCav2024sensors,dai2024networkedPredictiveControl}。但这些工作多关注车距调节、队列稳定性或单车轨迹跟踪，尚未把四车刚性载荷系统中的团队中心误差、车辆级短期路径、连接利用率、载荷受力、通信质量和执行器故障统一纳入一个可审计的预测控制闭环。

鲁棒 MPC、tube/convex robust DMPC、learning-supported MPC 和执行器 FDI/FTC 研究为不确定性、约束收缩和故障重分配提供了基础\cite{mayne2000constrainedMpc,mayne2005robustMpc,convexRobustDMPC2025ejc,gasparino2023nnMpcUncertainty,liu2024adaptiveFtcSbw,xu2024fdFtcSteering}。本文吸收这些研究中的约束可行性、退化保护和容错思想，但目标不是单车稳定或车队串稳定，而是高曲率路径与通信干扰耦合下的协同载荷安全。基于此，本文提出网络韧性 Koopman-时延一致性协同运输控制（\Method{}）；高速度实验仅作为敏感性与压力测试，不作为本文主要性能主张。
