...
STAGE = ROOT / "paper_dcn_tf12_draft" / "publication_readiness_round10_2026-05-11" / "e2_n20_comm_noise_ablation_merged"
...
def main() -> None:
    ...
    ax.set_title("E2 comm-only ablation, positive = worse than TF14-full (n=20)")
    ...
    save(fig, "R2_comm_module_ablation_heatmap")
    plt.close(fig)

    fig, axes = plt.subplots(1, 3, figsize=(12, 3.6))
    plot_methods = ["full_tf14", "no_comm_aware", "no_delay_compensation", "zoh_consensus_surrogate"]
    ...
    for ax, key, title in [
        ...
        (axes[2], "certificate_ok_ratio_mean", "certificate ok ratio"),
    ]:
    ...
    fig.suptitle("E2 communication-only module effects (high comm noise, n=20)")
    save(fig, "R2_comm_key_module_effects")
    plt.close(fig)
