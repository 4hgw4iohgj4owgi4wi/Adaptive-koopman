...
class NonlinearMPCController():
    ...
    def construct_controller(self, z_init, u_init, x_ref):
        ...
        setup_kwargs = {
            ...
            'scaling': int(self.solver_settings['scaling']),
        }


        # 仅当存在 linsys_solver 键时才添加该参数
    ...
    def update_solver_settings(self, solver_settings):
        ...
        if 'linsys_solver' in self.solver_settings:
            update_kwargs['linsys_solver'] = self.solver_settings['linsys_solver']
    ...
    def construct_objective_(self, xr):
        ...
        # 修改点：确保 xr 作为二维矩阵与 x_init 按列对齐相减
    ...
    #     Construct MPC objective function
    #     """
    #     # 确保 xr 是列向量以利用 numpy 广播机制
    #     xr_col = xr.reshape(-1, 1)
    #
    #     self._osqp_q[:self.nz * (self.N + 1) + self.nu * self.N] = np.hstack(
    #         [(self.C.T @ self.Q @ (self.x_init[:, :-1] - xr_col)).flatten(order='F'),
    ...
    def construct_constraint_matrix_data_(self, A_lst, B_lst):
        ...
        if sparse.issparse(self.C):
            C_dense = self.C.toarray()  # 转为稠密（24 x 92），维度不大，安全
        else:
            C_dense = self.C
        # 提取每列非零元素（原逻辑的稀疏友好版本）
        C_data = []
        for i in range(self.nz):
            col = C_dense[:, i]  # 取第 i 列
            nonzero_vals = col[np.nonzero(col)[0]]  # 提取非零值
            C_data.append(np.atleast_1d(nonzero_vals).tolist())
        # ====================== State variables ======================
        data = []
        A_inds = []
        start_ind_A = 1
        for t in range(self.N):
            for i in range(self.nz):
                # -1 for the -dz term, A_lst[t][:, i] for dynamics, C_data for state constraint
                data.append(np.hstack((-np.ones(1), A_lst[t][:, i], np.array(C_data[i]))))
                A_inds.append(np.arange(start_ind_A, start_ind_A + self.nz + 1 + len(C_data[i]) - 1))
        ...
        # ====================== Input variables ======================
        B_inds = []
        start_ind_B = start_ind_A + self.nz + sum(len(d) for d in C_data) - self.nz  # 调整偏移
        for t in range(self.N):
            for i in range(self.nu):
                data.append(np.hstack((B_lst[t][:, i], np.ones(1))))
                B_inds.append(np.arange(start_ind_B, start_ind_B + self.nz + 1 - 1))
                start_ind_B += self.nz + 1

        # Flatten data
        flat_data = [d for arr in data for d in np.atleast_1d(arr)]
        self._osqp_A_data = np.array(flat_data, dtype=float)
        self._osqp_A_data_A_inds = np.concatenate(A_inds).tolist() if A_inds else []
        self._osqp_A_data_B_inds = np.concatenate(B_inds).tolist() if B_inds else []
    ...
    def solve_mpc_(self):
        ...
        # if self.res.info.status != 'solved':
        #     print("OSQP status:", self.res.info.status)
        ...
        if self.res.x is None:
            print("OSQP status:", status)
            print("status_val:", self.res.info.status_val)
            print("iter:", self.res.info.iter)
            print("obj_val:", self.res.info.obj_val)
            print("prim_res:", prim_res)
            print("dual_res:", dual_res)
            raise ValueError('OSQP returned no solution!')

        if status == 'solved':
            pass
        elif status == 'solved inaccurate':
            print('[WARN] OSQP solved inaccurate, accepting solution.')
        elif status == 'maximum iterations reached' and prim_res < 1e-4 and dual_res < 1e-4:
            print('[WARN] OSQP hit max iterations, but residuals are small. Accepting solution.')
        else:
            print("OSQP status:", status)
            print("status_val:", self.res.info.status_val)
            print("iter:", self.res.info.iter)
