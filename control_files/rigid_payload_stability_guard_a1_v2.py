...
class RigidPayloadStabilityGuardA1V2:
    """A1-v2 guard with softer blending and progress-aware longitudinal action."""
    ...
    def __init__(
        ...
        *,
        beta_limit=0.22,
        r_limit=0.62,
        spread_delta_limit=0.080,
        spread_ax_limit=0.38,
        tracking_gain=None,
        stability_gain=None,
    ):
        ...
        if tracking_gain is None:
            tracking_gain = np.array([0.00, -0.82, -1.75, 0.00, -0.22, -0.30], dtype=float)
        if stability_gain is None:
            stability_gain = np.array([0.00, -0.62, -1.55, 0.00, -1.08, -1.35], dtype=float)
    ...
    def build_team_guard(self, team_state, team_ref):
        ...
        lag_s = float(np.clip(xr[0] - x[0], 0.0, 10.0))
        lag_vx = float(np.clip(xr[3] - x[3], -3.0, 3.0))

        u_track = np.array(
            [
                self.tracking_gain @ err,
                0.10 * lag_s + 0.85 * lag_vx - 0.12 * x[4],
            ],
            dtype=float,
        )
        u_stab = np.array(
            [
                ...
                -0.32 * abs(self._beta(x)) - 0.22 * abs(x[5]) + 0.38 * max(lag_vx, 0.0) + 0.05 * lag_s,
            ],
            dtype=float,
        )
        ...
        severity = max(beta_ratio, r_ratio, abs(err[1]) / 0.95, abs(err[2]) / 0.36)
        w_guard = np.clip((severity - 0.95) / 0.95, 0.0, 1.0)
        if beta_ratio > 1.35 or r_ratio > 1.35:
            w_guard = max(w_guard, 0.68)
